...
<p align="center">
  ...
  <a href="https://github.com/VibeBrains/VibeIDE/releases"><img src="https://img.shields.io/badge/версия-1.22.0-green.svg" alt="Версия" /></a>
...
- **Краткие ответы** — включены сразу: агент отвечает коротко, но каждый технический факт, код и текст ошибки остаются дословно; код, коммиты и документация пишутся обычной речью. Текст правил общий с VibeIDEA, действует и на внешних агентов по ACP. Уровень и остальные частые ручки — в меню быстрых настроек под полем чата.
...
- **Вшитый дизайн и тексты — не выглядят «сделанными нейросетью»** — дизайн-контекст проекта (для кого продукт + его палитра, гарнитуры и именованные правила) читается перед каждой правкой вида, а страницу автоматически замеряют 88 детерминированных проверок сразу на десктопе и телефоне 390px: и настоящие дефекты (контраст, зоны нажатия, обрезанный контент, битые картинки), и следы генератора (чип над заголовком, курсивный serif в герое, ореол, стекло, карточки-клоны, шаблонный текст), и русская типографика — висячие предлоги. Находки обведены прямо на странице — клик по рамке отправляет её в чат. Вкус отделён от дефекта: пол качества не отключается, а стилевую находку проект может объявить своей идентичностью — с причиной, которую видно через месяц. Работает и на dev-сервере фреймворка, не мешая горячей перезагрузке. Свои тексты — README, документацию, заметки к релизу — агент проверяет детектором нейрослопа без модели: каталог русских и английских примет общий с VibeIDEA, у каждой находки строка и способ исправить.
...
- **Node.js 24.18.0** (версия зафиксирована в `.nvmrc`) — именно этот мажор; на другом сборка падает, часто **молча**. **Начиная с версии 1.15.1** (обновление базы до VS Code 1.133) требуется 24; для более ранних релизов нужна была 22.22.1.
...
fnm install 24.18.0
fnm default 24.18.0
node -v        # должно показать v24.18.0
...
После установки тулчейна **открой новый терминал** (PATH обновится) и убедись, что Node = 24.18.0, прежде чем `npm install`.
