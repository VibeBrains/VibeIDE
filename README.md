...
<p align="center">
  ...
  <a href="https://github.com/VibeBrains/VibeIDE/releases"><img src="https://img.shields.io/badge/версия-1.14.0-green.svg" alt="Версия" /></a>
...
- 📱 **Агент в Telegram** — поставить задачу с телефона текстом или голосом, видеть прогресс и получить ответ; бот ваш собственный, без наших серверов ([как подключить](docs/manuals/telegramBridge.md)).
...
- **Агент с автопилотом** — доводит задачу до конца сам; мультиагентная оркестрация с блокировками; персистентные планы переживают перезапуск IDE; команда ролей-субагентов не теряет прогресс на лимитах — сохраняет сделанное и продолжает с места остановки (durable handoff); картинку в маршруте ролей система сама отдаёт роли на vision-модели (разбор скриншота → текст разработчикам), а на «продолжи» перед ответом запускает read-only разведчика по последним правкам и плану с подтверждением «оно/не оно». **«Проверить запуск»** показывает заранее, что агенту или роли будет разрешено — инструменты, закрытые пути, лимиты, модель, — ничего не запуская; после хода детерминированные проверки смотрят, что он **сделал**: секрет в изменённом файле, запись в закрытый путь, ссылка на несуществующую строку (модель для этого не вызывается вовсе). Панель **«Диспетчерская агентов»** отвечает, кто работает прямо сейчас и что сделали уже закончившие роли — цель, расход против квоты, изменённые файлы, итог и причина остановки словами; журнал прогонов переживает перезапуск IDE, а брошенный упавшим окном прогон видно отдельно, а не «потерялся».
...
- **Экономия токенов на выводе команд** — вывод терминала и MCP сжимается до попадания в контекст (command-aware профили git/тесты/ls/docker/find/install + дедуп шумных строк), ошибки и итоги сохраняются дословно; команда показывает, сколько токенов сэкономлено.
- **Spec-First и панель «Спеки»** — крупная фича сначала описывается спекой (`docs/specs/<id>/`: PRODUCT.md — поведение, TECH.md — реализация), панель в activity bar ведёт спеки со статусом (черновик → утверждена → реализована) и создаёт их «из задачи» через агента прямо из модалки со скрепкой; при реализации утверждённой спеки правки вне объявленной области ставятся на паузу. Рядом — панель «Документы» (markdown-дерево проекта).
- **Vibe Server — превью без деплоя** — локальный предпросмотр прямо в IDE: статика, dev-сервер фреймворка (Vite/Next/…) или Docker-окружение, живая перезагрузка, QR для телефона, ошибки превью одной кнопкой в чат агенту, клик по элементу страницы → селектор и заготовка правки сразу в чат.
- **Вшитый дизайн — интерфейс не выглядит «сделанным нейросетью»** — дизайн-контекст проекта (для кого продукт + его палитра, гарнитуры и именованные правила) читается перед каждой правкой вида, а страницу автоматически замеряют 55 детерминированных проверок сразу на десктопе и телефоне 390px: и настоящие дефекты (контраст, зоны нажатия, обрезанный контент, битые картинки), и следы генератора (чип над заголовком, курсивный serif в герое, ореол, стекло, карточки-клоны), и русская типографика — висячие предлоги. Находки обведены прямо на странице — клик по рамке отправляет её в чат. Вкус отделён от дефекта: пол качества не отключается, а стилевую находку проект может объявить своей идентичностью — с причиной, которую видно через месяц. Работает и на dev-сервере фреймворка, не мешая горячей перезагрузке.
...
- **Голосовой ввод офлайн** — диктовка в чат, редактор и терминал с распознаванием на своём компьютере (русский и английский), живой текст пока говоришь; звук никуда не отправляется.
- **Просмотр видео в чате** — `/watch <ссылка>`: IDE скачивает ролик, режет кадры по сменам сцен, берёт субтитры (или расшифровывает звук локально) и даёт разбор с тайм-кодами — модель видит экран, а не пересказывает субтитры.
...
Установщик не подписан сертификатом издателя, поэтому система один раз спросит подтверждение.

- **Windows:** в окне SmartScreen нажать «Подробнее» → «Выполнить в любом случае».
- **macOS 15 (Sequoia) и новее:** запустить приложение → «Готово» → **Системные настройки → Конфиденциальность и безопасность** → в разделе «Безопасность» нажать «Открыть всё равно» → запустить ещё раз. Control-клик → «Открыть» в этих версиях [больше не работает](https://developer.apple.com/news/?id=saqachfa).
- **macOS 14 и старше:** Control-клик по приложению → «Открыть» → «Открыть».

Подробно, включая способ через терминал и сверку контрольной суммы, — [docs/manuals/firstRun.md](docs/manuals/firstRun.md).
...
> **Окружение развивается — ваша копия может отстать.** В каждом релизе появляются новые скиллы и правятся существующие. Недостающие файлы дописываются при открытии проекта сами, а вот **уже существующий файл IDE не трогает** — иначе она затирала бы ваши правки. Поэтому файл, изменённый в новом релизе, остаётся у вас в старой редакции.
>
> Для этого есть две команды (меню «мозга» в заголовке окна или палитра):
> - **«Показать новое в окружении из релиза»** — отчёт, ничего не меняет.
> - **«Обновить окружение из релиза»** — отчёт плюс действия: **«Обновить безопасно»** (добавит новое и обновит то, чего вы не касались — потерять правки невозможно), **«Показать различия»** (дифф релиз ⇄ ваш, слияние руками), **«Оставить своё»** (зафиксировать свою версию) и **«Заменить всё»** (сброс к релизу, правки пропадут).
>
> IDE отличает «релиз обновился» от «вы правили» по `.vibe/.defaults.lock.json`, поэтому напоминание приходит **только когда сдвинулся релиз** — настроенные вами файлы поводом не считаются. Подробности: [`docs/manuals/vibeEnvironment.md`](docs/manuals/vibeEnvironment.md).
...
Полная спецификация формата (все поля с типами, инварианты, примеры) — [`docs/manuals/providersSpec.md`](docs/manuals/providersSpec.md). Её можно скопировать своей LLM и попросить собрать `.vibe/providers.json` под нужный провайдер.
...
Инструкции по сборке, гайдлайны и процесс PR: [CONTRIBUTING.md](CONTRIBUTING.md) и [docs/manuals/howToContribute.md](docs/manuals/howToContribute.md).
