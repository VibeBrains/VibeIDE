...
async function main() {
    ...
    if (process.env['BUILT_CLIENT']) {
        // Product version
        const version = await (0, zx_1.$) `node -p "require('../VibeIDE-win32-${arch}/resources/app/package.json').version"`;
        (0, codesign_1.printBanner)('Package client');
        const clientArchivePath = `.build/win32-${arch}/VibeIDE-win32-${arch}-${version}.zip`;
        await (0, zx_1.$) `7z.exe a -tzip ${clientArchivePath} ../VibeIDE-win32-${arch}/* "-xr!CodeSignSummary*.md"`.pipe(process.stdout);
