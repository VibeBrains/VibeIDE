...
const vscodeResourceIncludes = [
	...
	// VibeIDE — Font Awesome solid webfont (Command Center brain icon, etc.)
	'out-build/vs/workbench/contrib/vibeide/browser/media/fonts/*.woff2',
...
function patchWin32DependenciesTask(destinationFolderName: string) {
	...
	return async () => {
		...
		/* rcedit is Windows-only PE; bundled copilot/agent deps ship Linux/Darwin *.node beside win32 builds. */
		const deps = depsRaw.filter(depPath => {
			const n = depPath.replace(/\\/g, '/').toLowerCase();
			if (!n.endsWith('.node')) {
				return true;
			}
			if (n.includes('/prebuilds/')) {
				return n.includes('/prebuilds/win32-');
			}
			const nonWindowsBinary = /\b(linux|darwin|musl|android|freebsd)(\b|-|_)|-(linux|darwin)\b|x64-linux|linux-x64|arm64-darwin/u.test(n);
			return !nonWindowsBinary;
		});
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [vscode, vscodeMin] = ['', 'min'].map(minified => {
		...
		const destinationFolderName = `VibeIDE${dashed(platform)}${dashed(arch)}`;
