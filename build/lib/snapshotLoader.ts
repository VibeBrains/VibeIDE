...
export namespace snaps {
	...
	switch (process.platform) {
		case 'darwin':
			loaderFilepath = `VibeIDE-darwin/${product.nameLong}.app/Contents/Resources/app/out/vs/loader.js`;
			startupBlobFilepath = `VibeIDE-darwin/${product.nameLong}.app/Contents/Frameworks/Electron Framework.framework/Resources/snapshot_blob.bin`;
			break;

		case 'win32':
		case 'linux':
			loaderFilepath = `VibeIDE-${process.platform}-${arch}/resources/app/out/vs/loader.js`;
			startupBlobFilepath = `VibeIDE-${process.platform}-${arch}/snapshot_blob.bin`;
			break;
