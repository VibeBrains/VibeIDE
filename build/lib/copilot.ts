...
export function prepareBuiltInCopilotRipgrepShim(platform: string, arch: string, builtInCopilotExtensionDir: string, appNodeModulesDir: string): void {
	if (!fs.existsSync(path.join(builtInCopilotExtensionDir, 'package.json'))) {
		// Built-in Copilot extension not shipped (e.g. VibeIDE) — nothing to shim.
		return;
	}
	...
	// .moduleignore strips most of `@github/copilot/sdk` during packaging — ensure path exists before writing ripgrep shim.
