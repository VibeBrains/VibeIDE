...
// Вторая по частоте причина — не пакет вовсе.
//
// Зависимости ищутся регуляркой по ТЕКСТУ собранного бандла, и строковый литерал от кода она не
// отличает: оборот «from 'list'» в описании инструмента однажды уронил Фазу 1 сообщением
// «list импортирует out/main.js». Разбор AST это закрыл бы, но стоит дороже пользы, пока случай
// единичный. Что дёшево и обязательно — назвать эту возможность прямо здесь: без подсказки причина
// ищется с нуля каждый раз, а искать её приходится в момент релиза.
console.log('\nЕсли имени пакета не существует — проверьте, не текст ли это.');
console.log('Зависимости ищутся регуляркой по тексту бандла, и строку от кода она не отличает:');
console.log('оборот «from \'что-то\'» внутри описания инструмента или промпта читается как импорт.');
console.log('Лечится переформулировкой строки, а не установкой пакета.');
process.exit(1);
