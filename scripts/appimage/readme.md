...
# VibeIDE AppImage Creation Script

This script automates the process of creating an AppImage for the VibeIDE editor using Docker. It works on macOS and Linux platforms.
## Requirements
...
3.  **Copy Required Files:**
    ...
    *   `create_appimage.sh`
    *   `void.desktop` (already rebranded inside to VibeIDE labels)
    *   `void.png` or `vibeide.png`
...
5.  **Result:**
    ...
    After the script completes, it will generate an AppImage named `VibeIDE-x86_64.AppImage` (or similar, depending on your architecture) in the current directory.
...
*   **AppImage Creation:**
    *   Creates the `VibeIDE.AppDir` structure.
    *   Copies binaries, resources, and the `.desktop` entry.
    *   Copies `void.desktop` (VibeIDE branded) and icon file.
