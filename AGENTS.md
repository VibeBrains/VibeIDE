...
### Удаление релиза — сохранять и сливать changelog
Если релиз удаляется (`gh release delete`, например снят как дефектный), его **changelog НЕ теряется**: notes последующего релиза обязаны покрывать **всё с последнего ВЫЖИВШЕГО релиза**, а не только дельту от удалённого. Перед удалением сохранить notes удаляемого (в `D:\tmp\release-notes-vX.Y.Z.md` они уже лежат) и **влить** их в notes следующего (сгруппировать по секциям, убрать пункты, которые в новом релизе откатаны/неверны). Иначе код уезжает в бинарь, а его changelog нигде не опубликован. Пример промаха: 0.18.1 удалён, а D.16–D.26 чуть не остались без публикации в 0.18.2.
