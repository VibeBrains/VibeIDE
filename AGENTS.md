...
Все нетривиальные архитектурные находки, баги, решения и договорённости записываются в базу знаний `docs/knowledge/` (в репозитории), а не в приватную память агента. База разложена по доменам; единственный список записей — индекс [docs/knowledge/README.md](docs/knowledge/README.md).

После каждого ответа, где появились такие факты: добавить секцию `## [категория] Заголовок` (тело — **Контекст / Суть / Применение**) в подходящий тематический файл `docs/knowledge/<домен>/<файл>.md`, обновив существующую запись, если она есть. Новый файл — только под действительно новую категорию, и тогда **сразу строкой в индекс** (без неё запись не найдут; проверяется `npm run docs-graph-check`). Сообщить пользователю, что добавлено.
