...
/**
 * Full compile (`gulp compile`) writes nls.keys.json / nls.messages.json under `out-build/`.
 * Transpile-only dev (`out/main.js`) leaves them there — not next to `out/main.js`.
 * Without these files, resolveNLSConfiguration fails and UI stays English.
 */
function getNlsMetadataPath(mainBundleDir: string): string {
	const candidates = [
		mainBundleDir,
		path.join(mainBundleDir, '..', 'out-build'),
	];
	for (const dir of candidates) {
		if (
			fs.existsSync(path.join(dir, 'nls.messages.json')) &&
			fs.existsSync(path.join(dir, 'nls.keys.json'))
		) {
			return dir;
		}
	}
...
const nlsMetadataPath = getNlsMetadataPath(import.meta.dirname);
...
// Must match IProductService / EnvironmentMainService: in dev, vs/platform/product/common/product.ts
// appends " Dev" to nameShort, so getUserDataPath must use the same string or NLS clp lands in
// Roaming\<slug>-dev while the rest of the app uses Roaming\<slug>-dev-dev (broken RU UI).
const productNameShort =
	(process.env['VSCODE_DEV'] && product.nameShort
		? `${product.nameShort} Dev`
		: (product.nameShort ?? 'code-oss-dev'));
const userDataPath = getUserDataPath(args, productNameShort);
...
async function resolveNlsConfiguration(): Promise<INLSConfiguration> {
	...
	if (!userLocale) {
		const fallbackLocale = typeof product.defaultLocale === 'string' && product.defaultLocale.length > 0
			? product.defaultLocale.toLowerCase()
			: 'en';
...
function getUserDefinedLocale(argvConfig: IArgvConfig): string | undefined {
	...
	if (typeof product.defaultLocale === 'string' && product.defaultLocale.length > 0) {
		return product.defaultLocale.toLowerCase();
	}
