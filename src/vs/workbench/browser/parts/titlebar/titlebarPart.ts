...
/** VibeIDE chat container in secondary side bar; sidebar gear duplicates title-bar Manage */
const VIBEIDE_AUX_CONTAINER_ID = 'workbench.view.vibeide';
	...
	private createActionToolBarMenus(update: true | { editorActions?: boolean; layoutActions?: boolean; globalActions?: boolean; activityActions?: boolean } = true): void {
		...
		const updateToolBarActions = () => {
			...
			if (this.activityActionsEnabled) {
				...
				// VibeIDE: omit title-bar Manage (gear) while chat aux is visible — same affordance as view-title settings gear
				const activeAux = this.contextKeyService.getContextKeyValue<string>('activeAuxiliary') ?? '';
				const auxBarVisible = this.contextKeyService.getContextKeyValue<boolean>('auxiliaryBarVisible') === true;
				if (!(activeAux === VIBEIDE_AUX_CONTAINER_ID && auxBarVisible)) {
