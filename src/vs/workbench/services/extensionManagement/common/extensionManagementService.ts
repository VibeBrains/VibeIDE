...
import { createCommandUri, escapeMarkdownLinkLabel, IMarkdownString, MarkdownString } from '../../../../base/common/htmlContent.js';
...
export class ExtensionManagementService extends CommontExtensionManagementService implements IWorkbenchExtensionManagementService {
	...
	private galleryExtensionMarkdownLink(extension: IGalleryExtension): string {
		const rawName = extension.displayName?.trim() || extension.name?.trim() || extension.identifier.id;
		const label = escapeMarkdownLinkLabel(rawName);
		const href = extension.detailsLink?.trim();
		return href ? `[${label}](${href})` : label;
	}

	/** Prefer publisher display name; fall back so nls.format never receives undefined (shows as literal "undefined"). */
	private galleryPublisherMarkdown(p: { publisherDisplayName?: string; publisher?: string; publisherLink?: string; identifier?: IExtensionIdentifier }): string {
		const id = p.identifier?.id;
		const fromId = id?.includes('.') ? id.slice(0, id.indexOf('.')) : id;
		const display = (
			p.publisherDisplayName?.trim()
			|| p.publisher?.trim()
			|| fromId?.trim()
			|| localize('unknownPublisher', 'Unknown publisher')
		);
		const link = p.publisherLink?.trim();
		return link ? `[${escapeMarkdownLinkLabel(display)}](${link})` : escapeMarkdownLinkLabel(display);
	}
	...
	private async checkForTrustedPublishers(extensions: { extension: IGalleryExtension; manifest: IExtensionManifest; checkForPackAndDependencies: boolean }[]): Promise<void> {
		...
		if (untrustedExtensions.length === 1) {
			...
			if (otherUntrustedPublishers.length) {
				customMessage.appendMarkdown(localize('extension published by message', "The marketplace extension {0} is published by {1}.", this.galleryExtensionMarkdownLink(extension), this.galleryPublisherMarkdown(extension)));
				...
				if (otherUntrustedPublishers.length === 1) {
					customMessage.appendMarkdown(localize('singleUntrustedPublisher', "Installing this extension will also install [extensions]({0}) published by {1}.", commandUri, this.galleryPublisherMarkdown(otherUntrustedPublishers[0])));
				} else {
					customMessage.appendMarkdown(localize('message3', "Installing this extension will also install [extensions]({0}) published by {1} and {2}.", commandUri, otherUntrustedPublishers.slice(0, otherUntrustedPublishers.length - 1).map(p => this.galleryPublisherMarkdown(p)).join(', '), this.galleryPublisherMarkdown(otherUntrustedPublishers[otherUntrustedPublishers.length - 1])));
				}
				customMessage.appendMarkdown('&nbsp;');
				...
				customMessage.appendMarkdown(localize('message1', "The marketplace extension {0} is published by {1}. This is the first extension you're installing from this publisher.", this.galleryExtensionMarkdownLink(extension), this.galleryPublisherMarkdown(extension)));
			}
		} else {
			customMessage.appendMarkdown(localize('multiInstallMessage', "This is the first time you're installing extensions from publishers {0} and {1}.", this.galleryPublisherMarkdown(allPublishers[0]), this.galleryPublisherMarkdown(allPublishers[allPublishers.length - 1])));
		}
		...
		if (verifiedPublishers.length || unverfiiedPublishers.length === 1) {
			for (const publisher of verifiedPublishers) {
				...
				const publisherVerifiedMessage = localize('verifiedPublisherWithName', "{0} has verified ownership of {1}.", this.galleryPublisherMarkdown(publisher), `[$(link-external) ${URI.parse(publisher.publisherDomain!.link).authority}](${publisher.publisherDomain!.link})`);
			...
			if (unverfiiedPublishers.length) {
				...
				if (unverfiiedPublishers.length === 1) {
					customMessage.appendMarkdown(`$(${Codicon.unverified.id})&nbsp;${localize('unverifiedPublisherWithName', "{0} is [**not** verified]({1}).", this.galleryPublisherMarkdown(unverfiiedPublishers[0]), unverifiedLink)}`);
				} else {
					customMessage.appendMarkdown(`$(${Codicon.unverified.id})&nbsp;${localize('unverifiedPublishers', "{0} and {1} are [**not** verified]({2}).", unverfiiedPublishers.slice(0, unverfiiedPublishers.length - 1).map(p => this.galleryPublisherMarkdown(p)).join(', '), this.galleryPublisherMarkdown(unverfiiedPublishers[unverfiiedPublishers.length - 1]), unverifiedLink)}`);
				}
			}
		} else {
			customMessage.appendText('\n');
