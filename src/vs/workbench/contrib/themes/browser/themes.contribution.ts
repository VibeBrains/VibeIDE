...
registerAction2(class extends Action2 {
	...
	override async run(accessor: ServicesAccessor) {
		...
		const colorPickerPinnedIds = new Set<string>([
			ThemeSettingDefaults.VIBEIDE_DEFAULT_THEME,
			ThemeSettingDefaults.COLOR_THEME_LIGHT,
			...(productService.pinnedWorkbenchColorThemeSettingsIds ?? []),
		]);

		const lightEntries = toEntries(themes.filter(t => t.type === ColorScheme.LIGHT), localize('themes.category.light', "light themes"), colorPickerPinnedIds);
		const darkEntries = toEntries(themes.filter(t => t.type === ColorScheme.DARK), localize('themes.category.dark', "dark themes"), colorPickerPinnedIds);
		const hcEntries = toEntries(themes.filter(t => isHighContrast(t.type)), localize('themes.category.hc', "high contrast themes"), colorPickerPinnedIds);
...
registerAction2(class extends Action2 {
	...
	override async run(accessor: ServicesAccessor) {
		...
		const newThemeSettingsIds = new Set([ThemeSettingDefaults.COLOR_THEME_LIGHT, ThemeSettingDefaults.VIBEIDE_DEFAULT_THEME]);
		...
		const preferredId = (previousTheme.type === ColorScheme.LIGHT || previousTheme.type === ColorScheme.HIGH_CONTRAST_LIGHT) ? ThemeSettingDefaults.COLOR_THEME_LIGHT : ThemeSettingDefaults.VIBEIDE_DEFAULT_THEME;
		...
		disposables.add(picker.onDidAccept(() => {
			...
			(async () => {
				try {
					...
					await configurationService.updateValue(ThemeSettings.PREFERRED_DARK_THEME, ThemeSettingDefaults.VIBEIDE_DEFAULT_THEME);
				} catch (error) {
...
const productPinnedColorThemeDescriptions: Record<string, string> = {
	'vibe-neon': localize('vibeNeonThemePickerPinned', "VibeIDE default dark"),
	'vibe-neon-noglow': localize('vibeNeonNoGlowThemePickerPinned', "VibeIDE dark without editor glow"),
};

function colorThemePickerDescriptionForSettingsId(settingId: string | undefined): string | undefined {
	if (!settingId) {
		return undefined;
	}
	return productPinnedColorThemeDescriptions[settingId] ?? defaultThemeDescriptions[settingId];
}
...
function toEntry(theme: IWorkbenchTheme): ThemeItem {
	...
	const item: ThemeItem = {
		...
		theme: theme,
		label: theme.label,
		description: colorThemePickerDescriptionForSettingsId(settingId) ?? theme.description ?? (theme.label === settingId ? undefined : settingId),
	};
...
function toEntries(themes: Array<IWorkbenchTheme>, label?: string, pinnedIds?: ReadonlySet<string>): QuickPickInput<ThemeItem>[] {
	const effectivePinnedIds = pinnedIds ?? new Set([ThemeSettingDefaults.VIBEIDE_DEFAULT_THEME, ThemeSettingDefaults.COLOR_THEME_LIGHT]);
	const sorter = (t1: ThemeItem, t2: ThemeItem) => {
		const pin1 = effectivePinnedIds.has(t1.theme?.settingsId ?? '');
		const pin2 = effectivePinnedIds.has(t2.theme?.settingsId ?? '');
		if (pin1 !== pin2) {
			return pin1 ? -1 : 1;
		}
