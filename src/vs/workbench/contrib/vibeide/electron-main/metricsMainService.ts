...
// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/VibeIDE
	...
	async initialize() {
		...
		const { commit, version, vibeVersion, release, quality } = this._productService as any
		...
		console.log('User is opted out of basic VibeIDE metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('VibeIDE posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
