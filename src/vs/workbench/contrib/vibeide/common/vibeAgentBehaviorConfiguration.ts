...
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	...
	properties: {
		...
		'vibeide.coupling.historyDays': {
			...
			minimum: 7,
			maximum: 3650,
			description: localize('vibeide.coupling.historyDays', 'Связанность правок: за сколько дней истории искать файлы, которые обычно меняются вместе. Слишком широкое окно тянет в выводы связи давно переписанных модулей, слишком узкое не наберёт статистики.'),
		},
		'vibeide.coupling.maxCommits': {
			type: 'number',
			default: 2000,
			minimum: 50,
			maximum: 20000,
			description: localize('vibeide.coupling.maxCommits', 'Связанность правок: жёсткий потолок числа читаемых коммитов. Защищает от долгой паузы на репозитории с длинной историей.'),
		},
		'vibeide.coupling.maxFilesPerCommit': {
			type: 'number',
			default: 20,
			minimum: 2,
			maximum: 500,
			description: localize('vibeide.coupling.maxFilesPerCommit', 'Связанность правок: коммит, тронувший больше файлов, в подсчёте пар не участвует. Именно массовые коммиты (переименование, автоформат, обновление зависимостей) связывают всё со всем и потому не связывают ничего.'),
		},
		'vibeide.coupling.minPairCommits': {
			type: 'number',
			default: 3,
			minimum: 2,
			maximum: 50,
			description: localize('vibeide.coupling.minPairCommits', 'Связанность правок: сколько раз файлы должны измениться в одном коммите, чтобы считаться парой. Ниже трёх в список попадают случайные совпадения.'),
		},
		'vibeide.coupling.minPairRatio': {
			type: 'number',
			default: 0.3,
			minimum: 0.05,
			maximum: 1,
			description: localize('vibeide.coupling.minPairRatio', 'Связанность правок: какая доля правок файла должна быть совместной. Отсекает «сосед по частому коммиту» — файл, который меняется вместе со всеми просто потому, что его трогают постоянно.'),
		},
		'vibeide.coupling.bugWindowDays': {
			type: 'number',
			default: 180,
			minimum: 7,
			maximum: 1825,
			description: localize('vibeide.coupling.bugWindowDays', 'История починок: за какой период считать исправления багов по файлу. Починкой считается коммит, который и назван починкой, и правит код, — правка одних доков или одних тестов в счёт не идёт.'),
		},
		...
		'vibeide.providers.priceChangeWarningDays': {
			type: 'number',
			default: 14,
			minimum: 0,
			maximum: 365,
			scope: ConfigurationScope.APPLICATION,
			markdownDescription: localize('vibeide.providers.priceChangeWarningDays', 'За сколько дней предупреждать о подорожании модели, объявленном в файле провайдера (`costValidUntil` + `costAfter`).\n\nПредупреждение приходит один раз на набор моделей и называет коэффициент — «дороже в 10 раз» видно сразу. `0` отключает предупреждение; цена всё равно пересчитается в момент истечения, а отчёт о расходе останется верным.'),
		},
		'vibeide.hooks.trailLength': {
			type: 'number',
			default: 20,
			minimum: 0,
			maximum: 100,
			scope: ConfigurationScope.RESOURCE,
			markdownDescription: localize('vibeide.hooks.trailLength', 'Сколько недавних вызовов инструментов видит хук в поле `recent`.\n\nСлед нужен для правил на **последовательность**: «прочитать `.env`» и «запустить команду» порознь безобидны, а подряд — это эксфильтрация. Длиннее след — длиннее цепочка, которую можно описать правилом; `0` выключает поле совсем.\n\nЕдут только имена инструментов, пути-цели и сервер: аргументы и командные строки не передаются никогда.'),
		},
		'vibeide.hooks.trailMinutes': {
			type: 'number',
			default: 30,
			minimum: 1,
			maximum: 1440,
