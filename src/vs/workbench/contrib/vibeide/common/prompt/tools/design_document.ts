...
- 'uikit' — reads the project's own CODE and writes 'uiKit.md': a map of what ALREADY exists — declared tokens, component classes, exported components, and where each lives. Run this before building anything new: a model that does not know the names invents new ones, which is where an interface with five different buttons comes from. Needs no preview.
...
Requires an open preview for 'system' (same precondition as design_review). Without one it reports that nothing was measured rather than writing an empty system.`,
	params: {
		target: { description: `'product', 'uikit' or 'system'.` },
