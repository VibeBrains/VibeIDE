		...
		registry.registerConfiguration({
			...
			properties: {
				...
				'vibeide.agent.wakeOnBackgroundCommand': {
					type: 'boolean',
					default: true,
					description: localize('vibeide.agent.wakeOnBackgroundCommand', 'Будить агента, когда фоновая команда завершилась. Команда, запущенная с run_in_background, раньше сообщала о себе только опросом — и та, что закончилась, пока агент молчал, оставалась незамеченной до следующего вашего сообщения. При включённом флаге агент получает заметку о завершении: если он занят, она подмешивается к следующему шагу и работу не прерывает; если простаивает — начинается новый ход. Вывод команды в заметку не кладётся, агент читает его сам, если он нужен. Команда, которую агент убил сам, ничего не присылает. Выключите, если не хотите, чтобы агент оживал без вашего сообщения.'),
					scope: ConfigurationScope.APPLICATION,
				},
