...
import { VIBEIDE_ACCEPT_DIFF_ACTION_ID, VIBEIDE_REJECT_DIFF_ACTION_ID, VIBEIDE_GOTO_NEXT_DIFF_ACTION_ID, VIBEIDE_GOTO_PREV_DIFF_ACTION_ID, VIBEIDE_GOTO_NEXT_URI_ACTION_ID, VIBEIDE_GOTO_PREV_URI_ACTION_ID, VIBEIDE_ACCEPT_FILE_ACTION_ID, VIBEIDE_REJECT_FILE_ACTION_ID, VIBEIDE_ACCEPT_ALL_DIFFS_ACTION_ID, VIBEIDE_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
function getMountVibeCommandBar() {
	if (!mountVibeCommandBarPromise) {
		mountVibeCommandBarPromise = import('./react/out/vibe-editor-widgets-tsx/index.js').then(m => m.mountVibeCommandBar);
	}
	return mountVibeCommandBarPromise;
}



export interface IVibeideCommandBarService {
...
export const IVibeideCommandBarService = createDecorator<IVibeideCommandBarService>('VibeideCommandBarService');
...
registerSingleton(IVibeideCommandBarService, VibeideCommandBarService, InstantiationType.Delayed); // delayed is needed here :(
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		const mountVibeCommandBarPromise = getMountVibeCommandBar();
