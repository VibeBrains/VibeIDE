...
import { mountVibeSelectionHelper } from './react/out/vibe-editor-widgets-tsx/index.js';
...
export type VibeideSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.vibeSelectionHelper';
	// react
	...
	constructor(
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountVibeSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._vibeideSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies VibeideSelectionHelperProps)
