	...
	switch (basename) {
		...
		case 'constraints.json':
			return [
				...
				'- **`rules`** — массив правил:',
				'  - **`deny_write` + `pattern` + необязательный `message`** — блокирует любую запись агента: правку, перезапись, создание и удаление файла, если путь совпадает с **glob-подобным** шаблоном (`*`, `?`, `**`; совпадение по границам папок в любом месте пути).',
				...
				'  - Путь проверяется **развёрнутым**: `..` и символические ссылки правило не обходят — оно действует для каждого имени файла. На macOS и Windows регистр в шаблоне не важен. Полная спека — `docs/manuals/agentPathRulesSpec.md`.',
		...
		case 'permissions.json':
			return [
				...
				'- **Что проверяется.** Каждое чтение и каждая запись агента, включая создание и удаление файлов, и результаты поиска. Путь проверяется развёрнутым: `..` и символические ссылки правило не обходят. На macOS и Windows запреты не зависят от регистра, разрешения совпадают только точно.',
