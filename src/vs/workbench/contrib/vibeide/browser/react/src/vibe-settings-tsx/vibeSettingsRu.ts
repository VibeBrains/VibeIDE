...
export const chatS = {
	...
	quickSettingsTitle: 'Настройки чата',
	quickSettingsTooltip: (brevity: string) => `Настройки чата · ответы: ${brevity.toLowerCase()}`,
	quickSettingsAnswers: 'Ответы',
	quickSettingsAgent: 'Агент',
	brevityLabel: 'Краткость ответов',
	brevityTitle: 'Краткие ответы (vibeide.chat.brevity): технические факты остаются, уходит вода. Код, коммиты, документация и предупреждения пишутся обычной речью.',
	brevityChoice: (level: 'off' | 'lite' | 'full' | 'ultra') => ({ off: 'Выкл', lite: 'Мягко', full: 'Коротко', ultra: 'Предельно' })[level],
	responseLanguageLabel: 'Язык ответа',
	responseLanguageTitle: 'На каком языке агент отвечает (vibeide.agent.responseLanguage). «Авто» — по языку вашего сообщения.',
	responseLanguageChoice: (language: 'auto' | 'ru' | 'en') => ({ auto: 'Авто', ru: 'Русский', en: 'English' })[language],
	thinkingDisplayLabel: 'Мышление Claude',
	thinkingDisplayTitle: 'Что показывать из мышления Claude (vibeide.llm.claudeThinkingDisplay). Думает модель одинаково — меняется только то, что вы видите.',
	thinkingDisplayChoice: (display: 'summarized' | 'updates' | 'omitted') => ({ summarized: 'Сводка', updates: 'Шаги', omitted: 'Скрыто' })[display],
	minimalismLabel: 'Минимализм кода',
	minimalismTitle: 'Насколько агент экономен в коде (vibeide.global.minimalismMode): переиспользовать, а не писать заново; наименьшая правка, которая решает задачу.',
	minimalismChoice: (level: 'off' | 'lite' | 'full' | 'ultra') => ({ off: 'Выкл', lite: 'Мягко', full: 'Полный', ultra: 'Предельно' })[level],
	...
	subpinLabel: 'субпин.',
	subpinOffLabel: 'без субпин.',
	subpinOffHint: 'авто-резюм роли выключен',
	subpinTitle:
		'Авто-резюм субагента (vibeide.subagent.maxResumes): сколько раз VibeIDE САМ продолжит остановленную по лимиту роль (токены/шаги/время) с сохранённого места, прежде чем передать решение вам. 0 = не продолжать автоматически — сразу в ручной список «Продолжить роль». Субагентный аналог «подпин.» (авто-нудж основного агента). Дефолт 2.',
	resumeRoleLabel: (n: number) => `Продолжить роль${n > 1 ? ` (${n})` : ''}`,
	resumeRoleTitle: 'Есть остановленные роли, ожидающие продолжения с сохранённого места. Нажмите, чтобы выбрать и продолжить.',
	subagentSettingsTitle: 'Настройки субагентов: лимит шагов, токен-квота, авто-резюм, уведомления',
	...
	attachTitle: 'Прикрепить изображение или PDF (или вставить / перетащить)',
	voiceAria: 'Голосовой ввод',
	voiceStartTitle: 'Надиктовать сообщение (распознавание локально, без сети)',
	voiceStopTitle: 'Остановить диктовку (Esc — отменить без вставки)',
	voiceDownloadTitle: 'Скачать модели распознавания речи (один раз, ~{0} МБ)',
	voiceDownloadingTitle: 'Загрузка моделей… {0}%',
	runRouteAria: 'Выполнить маршрут ролей',
	runRouteTitle: 'Выполнить маршрут ролей: команда субагентов (планировщик → разработчики → ревьюер → QA → security) по вашей задаче',
	...
	switch (basename) {
		...
		case 'constraints.json':
			return [
				...
				'- **`rules`** — массив правил:',
				'  - **`deny_write` + `pattern` + необязательный `message`** — блокирует любую запись агента: правку, перезапись, создание и удаление файла, если путь совпадает с **glob-подобным** шаблоном (`*`, `?`, `**`; совпадение по границам папок в любом месте пути внутри проекта).',
				'  - **`deny_read` + `pattern`** — блокирует чтение: файл не отдаёт ни `read_file`, ни поиск, ни другие читающие инструменты. Чтобы просто убрать шум из контекста, удобнее **`.vibe/ignore`**.',
				'  - Путь проверяется **развёрнутым**: `..` и символические ссылки правило не обходят — оно действует для каждого имени файла. Шаблон без `/` в начале ищется внутри проекта на любой глубине, но не в папках над его корнем; с `/` в начале — от корня проекта. Регистр в запрете не важен. Полная спека — `docs/manuals/agentPathRulesSpec.md`.',
		...
		case 'permissions.json':
			return [
				'### Списки путей для агента (**permissions**)',
				'',
				'- **Что это.** Белые и чёрные списки путей для файловых инструментов агента: `allow_write`, `deny_write`, `allow_read`, `deny_read` — массивы glob-шаблонов (`*`, `?`, `**`).',
				'- **Как решается.** Запрет (`deny_*`) сильнее разрешения. Непустой `allow_*` — белый список: всё, что в него не входит, закрыто; пустой или отсутствующий список ничего не ограничивает. Запреты из `constraints.json` действуют отдельно, и разрешение здесь их **не отменяет**: путь должны пропустить оба файла.',
				'- **Что проверяется.** Каждое чтение и каждая запись агента, включая создание и удаление файлов, и результаты поиска. Путь проверяется развёрнутым: `..` и символические ссылки правило не обходят. Шаблоны считаются от корня проекта: `src/**` — это папки `src` внутри проекта, а не над ним. Запреты не зависят от регистра, разрешения совпадают только точно.',
				'- **Когда подхватывается.** Сразу после сохранения файла, перезапускать окно не нужно.',
				'- **Git.** По умолчанию файл **не** исключён из git. Если это ваши личные исключения, а не договорённость команды, добавьте `permissions.json` в `.vibe/.gitignore`.',
				'- **Полная спека** — `docs/manuals/agentPathRulesSpec.md`.',
			].join('\n');
...
export const VIBE_CONSTRAINTS_JSON_EXAMPLE = `${JSON.stringify(
	{
		vibeVersion: VV,
		_comment_file:
			'Жёсткие запреты для агента: deny_write — любая запись, deny_read — чтение и поиск. См. справку в форме.',
		rules: [
			{
			...
			},
			{
				...
				message: 'Внутренний аудит агенту не читать — ни чтением, ни поиском',
			},
		],
	},
	null,
...
export const VIBE_PERMISSIONS_JSON_EXAMPLE = `${JSON.stringify(
	{
		vibeVersion: VV,
		_comment_file:
			'Списки путей для агента. deny_* сильнее allow_*; непустой allow_* — белый список, всё остальное закрыто. Запреты constraints.json отсюда не отменяются.',
		allow_write: ['src/**', 'docs/**', 'tests/**'],
		deny_write: ['src/generated/**', '**/*.lock'],
...
export const miscS = {
	...
	mcpDriftPending: (tools: string) => `Инструменты изменились после одобрения: ${tools}. Агент их не видит, пока вы не посмотрите изменения.`,
	mcpDriftShow: 'Показать изменения',
	mcpDriftAccept: 'Принять',
	mcpToolWithheld: 'Изменился после одобрения — скрыт от агента до просмотра',
	mcpCommand: 'Команда:',
...
export const telegramS = {
	...
	voiceReady: 'Готово — голосовые распознаются локально, на этом компьютере.',
	voiceNeedsDownload: (mb: number) => `Нужно скачать ~${mb} МБ: ffmpeg и офлайн-модель распознавания. Скачается автоматически при первом голосовом — можно ничего не нажимать.`,
	voiceDownloading: 'Компоненты распознавания скачиваются…',
	voiceUnsupported: 'На этой платформе распознавание речи недоступно.',
	voiceDownloadNow: 'Скачать сейчас',
	voiceShared: 'Те же компоненты использует разбор видео (/watch) и диктовка в чате — второй раз не качаются.',
} as const;
