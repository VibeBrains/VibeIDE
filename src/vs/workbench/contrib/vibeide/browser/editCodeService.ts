...
// 		message: `VibeIDE Error: ${e.message}`,
	...
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(VIBEIDE_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		const greenBgClass = diff.enabled === false ? 'vibe-greenBG vibe-diff-disabled' : 'vibe-greenBG';
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(VIBEIDE_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(VIBEIDE_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._vibeideCommandBarService.stateOfURI[uri.fsPath];
