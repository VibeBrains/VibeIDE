...
/**
 * Desktop implementation of `ILLMMessageService` (contract lives in
 * `../common/sendLLMMessageService.ts`).
 *
 * The send path to every LLM provider runs in the main process; this class is the renderer-side
 * client of `vibeide-channel-llmMessage`. It uses the raw `IChannel` rather than `ProxyChannel`
 * because it needs `channel.listen` for the streaming callbacks (onText / onFinalMessage / onError)
 * that main pushes while a completion is in flight.
 *
 * Sits in `electron-browser/` because `IMainProcessService` is banned in `common/**` and
 * `browser/**`. Loaded from `vs/workbench/workbench.desktop.main.ts` — a browser-layer module
 * cannot import electron-browser.
 *
 * Open this file side by side with `electron-main/sendLLMMessageChannel.ts` — the other end.
 */
