...
/**
 * Desktop implementation of `IRemoteCatalogService` (contract lives in
 * `../common/remoteCatalogService.ts`).
 *
 * Fetches provider model catalogs. Network calls are routed to the main process over
 * `vibeide-channel-remoteCatalogFetch` via `IMainProcessService` (three call sites — catalog fetch,
 * dynamic-key probe, health check), which is why this class cannot live in `common/` or `browser/`.
 * `IRequestService` is still injected as the non-Electron fallback path.
 *
	...
	private async fetchOpenRouterCatalog(apiKey: string | undefined): Promise<RemoteModelInfo[]> {
		...
		return (data.data || []).map((model) => {
			...
			return {
				...
				floatsTo: floatingRefOf(model, id)?.target,
				floatKind: floatingRefOf(model, id)?.kind,
