...
export class ConsoleLogger extends AbstractLogger implements ILogger {
	...
	trace(message: string, ...args: unknown[]): void {
		if (this.canLog(LogLevel.Trace)) {
			...
			if (this.useColors) {
				logToConsole('log', '%cTRACE %c' + ts, 'color: #888', 'color: #888', message, ...args);
			} else {
				logToConsole('log', `[${ts}]`, message, ...args);
			}
		}
	}
	...
	debug(message: string, ...args: unknown[]): void {
		if (this.canLog(LogLevel.Debug)) {
			const ts = vibeTimestamp();
			if (this.useColors) {
				logToConsole('log', '%cDEBUG %c' + ts, 'background: #eee; color: #888', 'color: #888', message, ...args);
			} else {
				logToConsole('log', `[${ts}]`, message, ...args);
			}
		}
	...
	info(message: string, ...args: unknown[]): void {
		if (this.canLog(LogLevel.Info)) {
			const ts = vibeTimestamp();
			if (this.useColors) {
				logToConsole('log', '%c INFO %c' + ts, 'color: #33f', 'color: #888', message, ...args);
			} else {
				logToConsole('log', `[${ts}]`, message, ...args);
			}
		}
	...
	warn(message: string | Error, ...args: unknown[]): void {
		if (this.canLog(LogLevel.Warning)) {
			const ts = vibeTimestamp();
			if (this.useColors) {
				logToConsole('warn', '%c WARN %c' + ts, 'color: #993', 'color: #888', message, ...args);
			} else {
				logToConsole('log', `[${ts}]`, message, ...args);
			}
		}
	...
	error(message: string, ...args: unknown[]): void {
		if (this.canLog(LogLevel.Error)) {
			const ts = vibeTimestamp();
			if (this.useColors) {
				logToConsole('error', '%c  ERR %c' + ts, 'color: #f33', 'color: #888', message, ...args);
			} else {
				logToConsole('error', `[${ts}]`, message, ...args);
			}
		}
	}
