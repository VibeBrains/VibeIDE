...
/** Dev profile folder under APPDATA/.config (e.g. vibeide-dev). Was hardcoded code-oss-dev upstream. */
function devUserDataProfileSlug(productNameShort: string): string {
	const slug = productNameShort
		.trim()
		.toLowerCase()
		.replace(/\s+/g, '-')
		.replace(/[^a-z0-9-]/g, '')
