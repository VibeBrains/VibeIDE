...
export interface IProductConfiguration {
	...
	/** Additional color theme `settingsId` values pinned to the top of each section in the Color Theme quick pick (after built-in defaults). Product-specific (e.g. VibeIDE). */
	readonly pinnedWorkbenchColorThemeSettingsIds?: readonly string[];

	/**
	 * Default display language (e.g. `ru`) when neither `--locale` nor `locale` in argv.json is set.
	 * Requires bundled or installed language data for that locale.
	 */
...
export interface IProductOnboardingTheme {
	...
	readonly type: 'dark' | 'light' | 'hcDark' | 'hcLight' | 'neon';
}
