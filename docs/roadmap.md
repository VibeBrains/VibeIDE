...
- [ ] **Задел: батч-STT для английского в /watch** — вскрыто ревью 2026-07-18: STT-fallback идёт батчем (`transcribePcm16` → `decodeBatch`), батч требует **offline**-модель профиля, а en-профиль — NeMo streaming-only → английский подкаст без субтитров площадки абортится (`no offline model for batch transcription`). Нужна лёгкая offline en-модель в каталоге voice (зеркало `stt-models-v*`; принцип «для en хватит небольшой» — аналог whisper small из скилла) + гейт `isBatchTranscriptionAvailable` в renderer-фасаде (сервисный метод уже есть, но не вызывается никем — проверено grep'ом ревью).
