...
> Повод: обсуждение Obsidian/llms.txt → вопрос «а не сгенерировать ли индекс». Идея генерации **отвергнута по замеру** (frontmatter — 0 файлов из 96; колонка «О чём» — курированная проза, машинно не выводится: у `read-file-v2.md` заголовок ни о чём, а строка индекса несёт смысл). Но замер вскрыл настоящую проблему.

- [x] **Вскрыт дрейф: 4 списка одного множества** — ✅ (2026-07-15, next) `docs/knowledge.md` знал 11 доменов из 14; `docs/knowledge/README.md` не знал `tool-system/` (6 записей, заведён 2026-05-23 → **7 недель невидим**); `tool-system/README.md` перечислял 5 записей из 6; плюс `AGENTS.md` предписывал агенту старый плоский протокол («секцию `## [категория]` в конец `docs/knowledge.md`»), прямо **противореча** `CLAUDE.md` (домены + индекс). Корень — не небрежность, а дублирование индекса: расходятся по построению.
- [x] **«`vibe-docs-graph --check` — clean» (L157) оказалось ложью** — ✅ (2026-07-15, next) прогон на HEAD: **красный, 2 находки** (dead link `mdc:docs/knowledge.md` + сирота `edit-file-indentation-alignment.md`). Не знали, потому что скрипт **не подключён ни к CI, ни к `package.json`** — ран руками однажды. Плюс он был **слеп** к нашему классу поломки: `orphan = inDeg==0 && outDeg==0`, а файлы `tool-system/` ссылались друг на друга → кластер невидим из индекса, но не сирота. Плюс **шапка скрипта врала** («exit 1 if any dead links»), тогда как код гейтил и сирот. Три независимых способа соврать — в knowledge.
- [x] **Один индекс** — ✅ (2026-07-15, next) удалены `docs/knowledge.md` (огрызок-редирект) и `docs/references-v1/knowledge-md-contract.md` (нормировал формат `## <Topic> + Refs:`, отменённый декомпозицией 2026-05-09). `tool-system/README.md` **не удалён, а `git mv` → `overview.md`**: список тем (дублирующий) вырезан, карта кода слоя тулов и обоснование закалки сохранены как запись домена. `tool-system/` (7 строк) влит в общий индекс.
- [x] **Гейт членства в индексе** — ✅ (2026-07-15, next) `vibe-docs-graph --check` = dead links + сироты + **unindexed** (новый режим `--unindexed`); `mdc:`-ссылки исключены из резолва (схема продукта, резолвится от корня воркспейса → давали фантомный dead link). `npm run docs-graph-check` + workflow `knowledge-index.yml` (paths-gated, без `npm ci` — скрипт бездепный). **Проверен намеренной поломкой** (правило оракула, `patterns/agentic-rewrite-needs-oracle.md`): файл мимо индекса → exit 1; битая ссылка в индексе → exit 1; возврат → exit 0. Итог: `docs graph clean (100 files, all indexed)`.
- [x] **Зачистка ссылок на удалённый огрызок (17 мест)** — ✅ (2026-07-15, next) `.vibe-defaults/rules/knowledge.mdc` (**`alwaysApply: true`** — `mdc:`-ссылка резолвится в `<referenced_files>`, т.е. удаление файла оставило бы правило указывать в пустоту у каждого пользователя), `script-save.mdc` (ссылка на «Каталог скриптов» была **уже битой** — секция уехала в `bin-scripts.md` 2026-05-09), `clean-rules.mdc`, `roadmap-autopilot/SKILL.md`, `AGENTS.md` ×3 (снято противоречие с CLAUDE.md), `CLAUDE.md` ×2, `docs/README.md` ×2, `docs/references-v1/README.md`, `bin/{run-dev.bat,vibe-dev.bat,vibe-language-pack-nls.mjs}`, `scripts/vibe-dev.bat`, `build/lib/{electron.ts,preLaunch.ts}`, `src/…/{memoryLayerRouter,vibeProjectRulesService}.ts`, `.gitignore`, knowledge `{docs-layout,rule-link-resolution}.md`. Манифест перегенерён (`gen:all`). `docs/roadmap.md` не правил — это лог.
- [x] **Удалён `vibe doctor --knowledge`** — ✅ (2026-07-15, next) вместе с `scripts/lib/knowledge-md-staleness.cjs` + тестом. Причина: следил за **mtime огрызка** — файла, который по определению не менялся; при пороге 30 дней (mtime 28 мая) сегодня дал бы ложное «протухло» про заглушку, пока живая база правится ежедневно. Замена по mtime для 96 файлов бессмысленна — дрейф ловится гейтом членства, а не датой. Хвост: `patterns/lessons-from-roadmap-max-runs.md` приводит `knowledge-md-staleness` примером паттерна pure-helper+DI — пример остался как исторический, код удалён.
...
- [x] **Инсайты дня → knowledge** — ✅ дрейф NLS-индексов в dev (корень «сырого `{0}`» в диалоге сохранения) влит в `i18n/nls-indices.md`; прайс-API моделей + ловушка единиц каталожного cost — новая заметка `architecture/model-pricing.md`.
...
> Модалка «Проверка провайдеров» — brain-меню под «VibeIDE Команды», команда `vibeide.commands.checkProviders`. Дизайн, послойная модель и корень бага — `docs/knowledge/architecture/provider-diagnostics.md`. Resizable-окно зеркалит «VibeIDE Команды» (`commands-palette-modal.md`).
