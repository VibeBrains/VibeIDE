...
- [x] **Счётчик 36 → 9, девять сервисов, ~2100 строк, ноль тронутых потребителей** — ✅ (2026-07-15) по одному на коммит: `modelsDevCatalogStatusService` · `modelQuirksCatalogStatusService` · `vibeIdleWatchdogProxy` · `ollamaInstallerService` · `vibeideUpdateService` · `vibeideSCMService` (перенесён **целиком** `git mv` — файл самодостаточен: декоратор не инжектит никто снаружи, сервис и два `Action2` живут вместе) · `vibeServer/vibeServerService` (523 → 63 контракта + 489 impl) · `mcpService` (480 → 39 + 465) · `remoteCatalogService` (721 → 111 + 634). После каждого: compile + гейт регистрации + счётчик обязан упасть ровно на 3.
...
- [x] **Базлайн 9 зафиксирован как принятый долг** — ✅ (2026-07-15) `.claude/pipeline.md` Этап 2: «не хуже базлайна», проверять счётчиком `grep -c IMainProcessService` ≤ 9. **3 — апстрим VS Code, неустранимы** (`platform/browserElements/common/nativeBrowserElementsService.ts`: ни одного нашего коммита, ноль упоминаний `vibe`, пришёл с `Initial import`; правка = конфликт при мерже апстрима). **6 — наш отложенный долг** (см. ниже).
- [ ] **`sendLLMMessageService`** — отложен: **6 common-потребителей интерфейса**, целиком файл переносить нельзя. Схема применима, но требует аккуратности с контрактом.
