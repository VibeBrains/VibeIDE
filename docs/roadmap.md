...
- [ ] **Клик по элементу превью → селектор+правка в чат** — просили двое из трёх спикеров (один ищет «визуальный конструктор как в Тильде», второй руками копирует CSS-селекторы в заметки). У нас уже есть Vibe Server с превью: добавить режим «выбрать элемент» (аналог DevTools inspect) — клик по блоку превью отправляет в чат селектор + скриншот блока.
