...
- **Триаж всех шагов `pr.yml` на пред-существующие ошибки (2026-07-19)** — после snapshotLoader проверены остальные шаги job'а «Compile & Hygiene» локально на HEAD (в dependabot-прогоне не запускались — typecheck падал первым). Итог:
  ...
  - [ ] **`eslint` — КРАСНЫЙ (пред-существует):** 167 `header/header` ошибок в ~90 файлах (`scripts/*.js`, `cli-standalone/*.js`, 3× `src/*.js`) — отсутствует/неверен MS-заголовок. **Часть — конфиг, не долг:** eslint линтит и `node_modules/eslint/lib/*` (дыра в ignore). Требует решения: массовый прогон заголовков + чистка eslint-ignore. Не мелкий фикс.
