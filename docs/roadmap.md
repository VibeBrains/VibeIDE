...
- [ ] **R.2 — glob-scoped активация `.mdc`** — применять rule когда открытый/затронутый файл матчит `globs` (Cursor "Auto Attached"). **Фундамент готов:** `globs` уже парсятся, `decideRuleActivation` сейчас отправляет glob-правила в index (не инжектит всегда). Осталось: прокинуть контекст релевантных файлов (активный редактор / attached) в `getCombinedRules` и матчить `globs` через `base/common/glob`. Нужна сборка для проверки источника файл-контекста.
