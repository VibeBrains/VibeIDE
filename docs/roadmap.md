...
- [x] **Счётчик 36 → 6, десять сервисов, ~2500 строк, ноль тронутых потребителей** — ✅ (2026-07-15) по одному на коммит: `modelsDevCatalogStatusService` · `modelQuirksCatalogStatusService` · `vibeIdleWatchdogProxy` · `ollamaInstallerService` · `vibeideUpdateService` · `vibeideSCMService` (перенесён **целиком** `git mv` — файл самодостаточен: декоратор не инжектит никто снаружи, сервис и два `Action2` живут вместе) · `vibeServer/vibeServerService` (523 → 63 контракта + 489 impl) · `mcpService` (480 → 39 + 465) · `remoteCatalogService` (721 → 111 + 634) · `sendLLMMessageService` (453 → 51 контракта + 432 impl). После каждого: compile + гейт регистрации + счётчик обязан упасть ровно на 3.
...
- [x] **Базлайн 6 зафиксирован как принятый долг** — ✅ (2026-07-15) `.claude/pipeline.md` Этап 2 (файл локальный — `.claude/` в `.gitignore`; долговечная запись в knowledge): «не хуже базлайна», проверять счётчиком `grep -c IMainProcessService` ≤ 6. **3 — апстрим VS Code, неустранимы** (`platform/browserElements/common/nativeBrowserElementsService.ts`: ни одного нашего коммита, ноль упоминаний `vibe`, пришёл с `Initial import`; правка = конфликт при мерже апстрима). **3 — наш отложенный долг** (`metricsService`, см. ниже).
...
- [ ] **`metricsService`** — отложен: **54 вызова `capture()` в 12 файлах**, `LLMMessageChannel` держит `MetricsMainService`. Сток пустой (`NoOpMetricsClient`, PostHog выпилен) — но инструментовка живая и есть готовый сигнал качества. Делать вместе с переименованием `metrics` → `routingOutcomeLog` (см. вектор 1 ниже).
