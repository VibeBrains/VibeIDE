...
- [ ] **Панель «Спеки»: дрейф-от-спеки (Phase C)** — 📋 план готов (2026-07-13, next), реализация следующим заходом. Цель: тред, реализующий approved-спеку, при правке файла вне объявленной области → пауза (переиспользовать механику план-дрейфа). **Зафиксированные решения:** (1) область спеки — явный `scope:` (список globs) во фронтматтере PRODUCT.md, НЕ эвристика по TECH.md-прозе; (2) дрейф срабатывает ТОЛЬКО при явной привязке треда действием «Реализовать спеку» (не на любой approved-спеке — иначе шум). **План:** новое поле `scope`/`boundThreadId` во фронтматтере + парс в `IVibeSpecsService` (`specForThread`, `isPathInScope`); действие «Реализовать спеку» (для `status:approved`) пишет `boundThreadId` + промпт на implement-specs (паттерн Phase B); `_pauseForSpecDrift(threadId, editedUri)` зеркалом `_pauseRunningPlanStepForToolDrift` на хук-точках `chatThreadService.ts:4998` и `:7136` (только edits-класс, `uri` из toolsServiceTypes.ts:68,73); настройка `vibeide.specs.driftPause` (always/manual-only/never, default manual-only) рядом с `vibeide.plans.toolDriftPause`; UI-чип паузы переиспользуется (реагирует на `step.status==='paused'`). **Грабля:** чип завязан на шаг плана — спек-дрейф без активного плана даёт только warning-тост (потому и триггерим только внутри implement, где план обычно есть). Механика план-дрейфа — `docs/knowledge/` (разведка 2026-07-13).
