...
- [ ] **Pin задачи в `convertToLLMMessage`-усечении**: payload-truncation (~150k→~10k) выкидывает первое user-сообщение → модель «теряет» исходную задачу (#013: не смогла назвать первый промпт). Перенести anchor-pin из thread-trim (#012) и на payload-уровень — закрепить первое user-сообщение в усечении.
- [ ] **Cap главных поисков как у grep**: `search_for_files` (textSearch) / `glob` / `search_pathnames_only` всё ещё `CancellationToken.None` — на огромном репо могут висеть (grep уже имеет 15с-cap). Для консистентности и защиты EH добавить тот же 15с-cap.
