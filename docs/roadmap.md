...
### Фикс сборки: голосовой воркер не попал в macOS-бандл 1.9.0 → VERIFY-GATE целостности (2026-07-24, ветка `main`)

- [x] **Голосовой ввод молчал на macOS 1.9.0 — воркер отсутствовал в `.app`** — ✅ (2026-07-24, main) по репорту владельца: микрофон в системной панели горит, но в IDE запись не включается и голос не снимается. Диагноз по логам установленного `.app` (`~/Library/Application Support/VibeIDE/logs/.../main.log`): `[vibeVoice worker] ERR_MODULE_NOT_FOUND: .../vibeide/node/voice/vibeVoiceWorkerMain.js` + renderer `engine warm-up is slow — dropping oldest queued audio`. Это НЕ разрешение микрофона: `getUserMedia` даёт доступ (индикатор горит, аудио копится), но форк воркера-распознавателя падает — модуля нет в бандле. Проверено фактом: в `/Applications/VibeIDE.app` у `vibeide` есть только слой `browser`, весь `node`-слой отсутствует; при этом все соседи по списку `workbenchDesktop` (`debug/node/telemetryApp`, `watcherMain`, `ptyHostMain`, `agentHostMain`, `diffWorkerMain`) на месте — выпал только voice. Регистрация воркера на теге `v1.9.0` корректна (`build/buildfile.ts:30`), voice-коммит — предок тега, полная компиляция всегда `rimraf out-build` + пересборка (`build/gulpfile.compile.ts:14`), локальный `out-build` воркер содержит. Значит артефакт 1.9.0 упакован из **неполного `out-build`** (частичная/устаревшая сборка), а `release-macos.sh` это молча допускал (`--skip-publish` разрешает stale out-build). **Корневой фикс — VERIFY-GATE целостности бандла** в обоих скриптах (`release-macos.sh` после `gulp vscode-darwin`, `release-windows.ps1` после `gulp vscode-win32`): именованный список обязательных VibeIDE node-воркеров (сейчас `vibeVoiceWorkerMain.js`) проверяется в собранном пакете ДО подписи/публикации, отсутствие → `die`/`throw` с указанием пересобрать полной Фазой 1. Ловит любую причину пропажи (stale out-build, молчаливый сбой esbuild, будущие воркеры). Выпуск — mac-only `1.9.1` полной пересборкой. Инсайт (форкнутый воркер = отдельная bundle-точка; наличие в `out-build` ≠ наличие в `.app`) — `docs/knowledge/build/macosToolchain.md`. bash-синтаксис скриптов проверен (`bash -n`); PowerShell локально не проверялся (нет `pwsh` на маке) — правка по существующим паттернам скрипта.
