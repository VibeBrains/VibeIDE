...
> ✅ **РЕЛИЗ 1.7.0 ОПУБЛИКОВАН (2026-07-12, тег `v1.7.0`, mac DMG+ZIP).** merge `next`→main → Фаза 1 → Фаза 2 (`--skip-compile`, по «гуд») → курированные ноты (`gh release edit --notes-file`, блок поддержки, фраза №7 «по следам»), фраза в Историю v1.7.0. Фича-релиз: vision-маршрутизация ролей (картинка→дизайнер на vision-модели, авто-фолбэк, скрепка/дроп/пейст в маршруте), авто-разведчик на continuation-запросах (гейт «оно/не оно» в треде), упрощённый/полный вид панели чата (иконка-глаз, дефолт упрощённый), тултипы+перенос кнопок субагента/разведки к модели. 22 коммита с базлайна 1.6.0. Базлайн ченджлога → 1.7.0. Кросс-платформенно: mac здесь, Windows догоняет той же версией.
