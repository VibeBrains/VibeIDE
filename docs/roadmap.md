...
### Инфраструктура
- [ ] **CI: погасить скипы тестов — вернуть покрытие (долг PR #2)** — ⏳ скип в CI = заплатка, не решение (CI существует, чтобы гонять тесты, а не скипать). Реестр и правильные фиксы — `testing/ciCoverageSkips.md`. Погасить: (1) **policyExport** — отдельный `--user-data-dir` для вложенного Electron (наиболее вероятная причина зависания) + разобрать автономный экспорт (renderer-наблюдаемость через fs-маркеры); снять `GITHUB_ACTIONS`-скип. (2) **Component Screenshots** — свой baseline (checked-in эталоны + `component-explorer render` без внешнего hediet); вернуть auto-run вместо `workflow_dispatch`. НЕ трогать: vibeDocsGraph renderer-скип (корректный роутинг в node-раннер), react/out-исключения.
