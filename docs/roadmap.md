...
- [x] **R.4 — enable/disable правил (toggle)** — ✅ (2026-05-30, command-форма): per-workspace disabled-set (persisted `IStorageService`, `StorageScope.WORKSPACE`), чтится в `_combineSources` (отключённые не инжектятся и не индексируются). Команда палитры **«VibeIDE: Toggle Project Rule»** — quick-pick источников с ✓/⊘ и переключением. `isRuleEnabled`/`setRuleEnabled` в интерфейсе. **R.4.1 (polish):** полноценная React-панель в Vibe Settings (список + превью + тоггл) — поверх этих же API; нужна сборка.
