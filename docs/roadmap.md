...
- [ ] **LLM-суммаризация истории чата** — 📋 планируется. Сейчас переполнение головы истории сворачивается ТЕКСТОВО (нарезка `content.slice` в `<chat_summary>`, convertToLLMMessageService:~2334). Настоящая семантическая суммаризация = отдельная крупная фича (инкрементальный кэшированный summary на треде, триггер по переполнению, cache-friendly — иначе бьёт prompt-кэш и стоимость каждый ход). НЕ заглушка в hot-path; требует своего плана. Прецедент осторожности: текущий budget-fill специально ушёл от «раздавить в стаб» ради fidelity и кэша.
