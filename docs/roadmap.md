...
- [ ] **Роутер, обучающийся на телеметрии** — надстройка над живым `common/modelRouter.ts` (сейчас чисто rule-based: роли/лимиты/цена). Два принципа из прототипа: `finalScore = baseScore + learnedAdjustment` (поправки **поверх** правил, а не замена ML-чёрным ящиком) и явный **abstain** (`shouldAbstain` + `abstainReason`) вместо тихого «выбрал хоть что-то». Нерешённое в прототипе — персистентность поправок. Предусловие: сперва должно быть что мерить (прототип умер именно от того, что телеметрию завели раньше потребителя).
