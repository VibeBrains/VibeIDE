...
- [/] **Фаза 1 (MVP)** — пункт меню + команда; resizable-модалка со списком *активных* провайдеров (резолвится API-ключ: gui / `.vibe/.env` / `apiKeyRef`; встроенные + `.vibe/providers.json`); послойная «Проверить связь» — L1 конфиг / L2 сеть / L3 авторизация / L4 модели (через `IRemoteCatalogService.fetchDynamicWithStatus`/`fetchCatalog`: latency, число моделей, наличие выбранной модели в каталоге); просмотр/смена токена + ре-probe; **«Сбросить клиентов провайдера»** — новый IPC в electron-main: чистит `openAIClientCache`/`ollamaClientCache` (локальные) + **пересоздаёт shared undici-dispatcher** `ensureSystemCADispatcher` (облачные — корень кейса «менял провайдеров, помог только рестарт») + ресинк `_externalProviders`, без перезапуска IDE; сводка-заголовок; базовый MD-экспорт (секреты маскируются). Сетевые вызовы — через electron-main каналы; env-ключи не покидают main.
- [ ] **Фаза 2 (глубина)** — L5 сквозной тест через настоящий `sendLLMMessage` (opt-in, «тратит токены») — ловит именно баг кэша, который probe не видит (probe идёт мимо SDK-кэша); диагностический кольцевой лог-буфер, перехватывающий события send-path (sync провайдеров, cache hit/miss, IPC-send, установка aborter, first-token, ошибки/таймауты — точки в `sendLLMMessage.impl.ts` / `sendLLMMessageChannel.ts`); расширенный MD-экспорт с логами (секреты вырезаются); авто-подсказка сброса, когда L3/L4 зелёные, а L5 красный.
- [ ] **Фаза 3 (полировка)** — авто-рефреш, история latency, drill-down по слоям, copy-per-provider, индикатор здоровья в провайдер-статус-баре.
- [ ] **Durable-фикс (вне модалки)** — две ветки молчаливого отказа до перезапуска: (1) **облако** — process-wide `sharedDispatcher` (`aiSdkAdapter.ts:81`, захвачен в module-const) залипает → health-check/пересоздание пула; (2) **локальные** — `openAIClientCache`/`ollamaClientCache` (`sendLLMMessage.impl.ts:42,48`) не инвалидируются при смене `settingsOfProvider`/`.vibe/providers.json` → версия конфига в ключе кэша ИЛИ подписка на смену настроек. Кнопка «Сбросить клиентов» остаётся safety-net. Детали — `docs/knowledge/architecture/provider-diagnostics.md`.
