...
### Инфраструктура
- [~] **CI: Electron integration-тесты (PR #2, 1/2)** — 🚧 (2026-07-21, ветка `next`) после зелёных юнитов вскрылся integration-слой, 2 фейла. **`Disk File Service > stat - executable` — ПОЧИНЕН:** фикстура `fixtures/executable/executable` потеряла executable-бит в git (100644 вместо 100755; `core.fileMode=false` на dev-машине скрыл потерю), восстановлено `git update-index --chmod=+x`, проверено (187 passing, stat-executable ✔). **`PolicyExport > exported policy data` — ОТКРЫТ (глубокая нора):** тест запускает ВЛОЖЕННЫЙ полный Electron (`code.sh --export-policy-data`) для проверки актуальности `policyData.jsonc`, виснет 60с. Диагноз: (1) в тесте вложенный Electron наследует env родительского тест-VS-Code → вероятен lock-конфликт общего user-data-dir → зависание (фикс-кандидат: отдельный `--user-data-dir` в exec теста, `policyExport.integrationTest.ts:56`); (2) автономно экспорт не пишет файл (exit ~14с, лог «Export started» не появляется → contribution на `WorkbenchPhase.Eventually`/`withProgress(Notification)`/`whenInstalledExtensionsRegistered` не доходит). **Преграды диагностики:** contribution в renderer-процессе → `console.*` не в stdout даже с `ELECTRON_ENABLE_LOGGING=1`; `logService`-лог-файл не найден; локальная пересборка нестабильна. **След. заход:** fs.appendFileSync-маркеры для наблюдаемости ИЛИ найти user-data-dir/logs; если вложенный Electron фундаментально хрупок в headless — скип в CI как для ADO (`TF_BUILD`).
