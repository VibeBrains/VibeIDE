...
- [ ] **Задел: чисто аудио-вход для /watch (подкаст, mp3, голосовое)** — вопрос из комьюнити 2026-07-18. Сейчас IDE-пайплайн упадёт на аудио-only источнике: формат `bv*[height<=…]+ba/b[…]` требует видеодорожку, а стадия кадров бессмысленна. Нужна ветка: детект аудио-входа (probe: нет видеопотока / расширение mp3-wav-m4a-opus) → yt-dlp `-x` (или локальный файл сразу в ffmpeg) → PCM16 → существующий STT-fallback (GigaAM батч) или субтитры площадки → запрос БЕЗ вложений-кадров, промпт без «что на экране». Референс поведения — скилл-аналог `/watch` у Claude: аудио-ветка добавлена и проверена фактом 2026-07-18 (yt-dlp `-x` → mp3 без видеодорожек → whisper srt). Vision-гейт для аудио не нужен — пропускать.
