...
- [ ] **LLM-суммаризация истории чата** — 📋 план готов (2026-07-13), реализация следующим заходом. Сейчас переполнение головы сворачивается ТЕКСТОВО (`content.slice` → `<chat_summary>`, convertToLLMMessageService:~2292–2356 через `planBudgetFillTail`). **План (инкрементальный кэшированный summary на треде, вне hot-path):**
    ...
    - **Развилки на решение:** (1) фон после хода vs лениво на переполненном запросе — рекоменд. фон; (2) инкрементально vs полный пересчёт головы — рекоменд. инкрементально + периодический full; (3) aux/дешёвая модель vs чат-модель — рекоменд. aux. Прецедент осторожности: budget-fill специально ушёл от «раздавить в стаб» ради fidelity/кэша — не регрессировать.
