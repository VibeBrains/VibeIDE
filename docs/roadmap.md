...
- [ ] **5 настоящих orphan-reads (пред-существующие, разные фичи)** — гейт теперь корректно ловит незарегистрированные ключи: `vibeide.git.githubToken`/`repoSlug`/`defaultBranch` (`vibeJobPRCompletionService` — `githubToken` секрет, не должен быть plain-настройкой; остальные — зарегистрировать или читать из проектного конфига), `vibeide.privacy.strict` (`vibeProviderProxyService`), `vibeide.trustScore.level` (`vibeCommands` — **читается и пишется** `updateValue` без регистрации схемы). Не экземптить (спрячет баги) — зарегистрировать/перевести на корректный источник. Не блокирует эту ветку по существу (долг других фич), но держит гейт красным до разбора.
