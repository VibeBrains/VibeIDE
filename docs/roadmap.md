...
- [x] **R.9 — legacy `.cursorrules`** — ✅ (2026-05-30): добавлен в `RULE_FILE_NAMES` (плоский always-on источник) — для проектов, импортированных из Cursor (single-file convention до `.mdc`). Вотчер покрывает (`e.contains`).
...
- [ ] **R.11 — конфигурируемые лимиты** — вынести `MAX_RULE_FILES`/`MAX_RULE_FOLDER_DEPTH`/`MAX_RULE_FILE_BYTES` в `vibeide.rules.*` settings (сейчас хардкод-константы).
- [ ] **R.12 — UX панели + `@rule`-фидбек** — бейдж режима активации (always/trigger/glob/agent-requested) у каждого правила в `ProjectRulesPanel`; тост при `@rule:<неизвестное>`.
