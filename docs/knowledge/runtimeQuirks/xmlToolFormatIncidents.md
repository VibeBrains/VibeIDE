...
- [xmlToolNormalization.md](../architecture/xmlToolNormalization.md) — pipeline overview
...
| **Осталось (follow-up)** | (а) ✅ `rewrite_file` truncation-guard сделан (см. `toolSystem/editSafety.md`); (б) промпт-нудж против `__patch`-скриптов — не сделан |
...
1. **Скопировать template** — `docs/knowledge/_templateIncident.md` (краткая форма) или существующий блок выше (расширенная таблица form).
