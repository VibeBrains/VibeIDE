...
## [Footgun] Необязательный второй параметр у функции, которую зовут через `.map`

**Контекст:** у `isContinuationRequest(text)` появился порог вторым аргументом —
`isContinuationRequest(text, maxWords = 5)`. Тесты звали её как `список.map(isContinuationRequest)`,
а `map` передаёт колбэку **три** аргумента: значение, **индекс**, массив. Порогом молча стал индекс:
первый элемент проверялся с порогом 0, второй — с 1, и результат менялся от позиции в списке.
Поймали тесты, но только потому, что список был длиннее одного элемента.

**Правило:** функция, которую передают в `map`/`filter`/`forEach` по имени, не должна принимать
второй параметр примитивного типа. Настройка передаётся **объектом** (`{ maxWordsBeyondPhrase }`) —
тогда `.map(fn)` не компилируется вовсе: `number` не приводится к объекту, и TypeScript называет
ошибку раньше, чем она превратится в зависимость результата от порядка элементов.

**Обратная сторона:** обёртка `.map(x => fn(x))` тоже решает проблему в конкретном месте, но
защищает только то место, где о ней вспомнили. Сигнатура защищает все.
