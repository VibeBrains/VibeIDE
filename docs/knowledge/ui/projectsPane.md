...
## [грабля] Строка-панель с flex + горизонтальным padding ОБЯЗАНА быть `box-sizing: border-box`

**Контекст:** 2026-07-21 — пользователь: «в панели VibeProjects поехали иконки», корзина (`Codicon.trash`, «Удалить из списка») в строке проекта обрезалась справа наполовину.

**Суть:** `.vibe-projects-slot-row` в [media/vibeide.css](../../../src/vs/workbench/contrib/vibeide/browser/media/vibeide.css) имел `width: 100%` + `padding: 0 12px` **без** `box-sizing: border-box`. Content-box: реальная ширина = 100% + 24px, приколотый вправо блок действий (карандаш + корзина) выезжает за видимый край `.monaco-list-row` ровно на величину padding'а — обрезается последняя иконка. Фикс — `box-sizing: border-box`.

**Это ВТОРОЕ появление той же ловушки** — первое в панели «Спеки» (пилюля `S`, см. [specsPane.md](specsPane.md), «Последняя пилюля обрезалась справа»). Паттерн: любая новая строка панели с `display:flex` + `width:100%` + горизонтальным padding обязана быть `border-box`, иначе последний правый элемент режется.

**Применение:**
- В смоуке проверять именно **последний** правый элемент строки, а не предпоследний.
- Проверка фактом без глаз: замер геометрии через CDP — `trash.getBoundingClientRect().right > listRow.getBoundingClientRect().right` = обрезка. Временный возврат `content-box` в рантайме воспроизводит overflow (доказывает причину). Подтверждено 2026-07-21: overflow ровно 12px (= padding справа).
...
## [foot-gun] Классы в React-разметке префиксуются сборкой, CSS — нет

**Симптом:** новый класс подсветки не применялся. Рендер вызывался (проверено логом), CSS-правило было в собранных стилях, в DOM — ничего. Три попытки чинить «гонку с виртуализацией» ушли впустую, потому что ломалось не там.

**Причина:** React-часть собирается через `scope-tailwind ./src -o src2/ -s vibe-scope -c styles.css -p "vibe-"` (см. `react/build.js`). Флаг `-p` добавляет префикс `vibe-` **классам в разметке**, а `styles.css` переносится как написан. Класс `vibe-revealed-message` в JSX превратился в `vibe-vibe-revealed-message` и не совпал с селектором.

**Правило:**

| Где | Как писать |
|---|---|
| JSX (`className`) | **без** префикса: `revealed-message` → в бандле станет `vibe-revealed-message` |
| `styles.css` | **с** префиксом: `.vibe-revealed-message` |

**Как поймать за минуту:** после `npm run compile` посмотреть регенерированный `react/src2/**` — там лежит именно то, что уйдёт в бандл. `grep` по своему классу в `src2` отвечает на вопрос «что реально отрендерится» точнее любых рассуждений о React.

**Методический вывод:** когда рендер доказанно происходит, а DOM пуст — виновата сборка, а не логика. Следующий шаг после такого расхождения — смотреть артефакт сборки, а не переписывать компонент.

## Заявка про «чужую» кнопку оказалась про свою (2026-08-12)

В роадмапе с 09.08 жил пункт: апстримная искра Copilot в поле ввода SCM видна всегда, потому что
рисуется по `ChatContextKeys.Setup.*`, а у нас сетап не завершается никогда. Пункт читался
убедительно: ссылка на строку апстрима, разобранное условие показа, известная побочка
`setForceHidden`.

Кнопку погасили CSS-правилом. Живая проверка показала, что в поле ввода SCM **ровно одна** искра
и её доступное имя — «VibeIDE: Generate Commit Message». То есть погасили СВОЮ генерацию
сообщения коммита, а копилотовской кнопки там нет вовсе.

Что из этого следует для работы:

- **Подпись элемента — единственный надёжный признак.** Иконка общая (`codicon-sparkle`
  используется и нами, и апстримом), селектор по ней задевает своё же. Первым делом читать
  `aria-label`, а не класс глифа.
- **Заявка в роадмапе — гипотеза, а не факт**, даже когда в ней есть номер строки апстрима.
  Проверять до правки: типы, слои и сборка на такой ошибке молчат все три.
- Проверять обеими сторонами: с правилом `display:none` и 0×0, после снятия `display:flex` и
  22×22 — иначе «стало не видно» неотличимо от «его там и не было».
