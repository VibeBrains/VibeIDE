...
## [метод] Перенос чужой палитры в тему редактора — перекрашивание по якорной шкале

**Контекст:** 2026-08-11, четыре тёплые темы перенесены из палитры веб-панели VibeSter.

**Задача, которая выглядит копированием, но им не является.** Палитра панели — около сорока
переменных на половину (фон, карточка, primary, border, sidebar). Тема редактора — двести
поверхностей **и подсветка кода**, которой в панели нет вовсе. Один к одному не ложится.

**Что работает:** взять готовую тему-донор (у нас Graphite) как **структуру светлот** — она
несёт выверенные контрасты, — и подставить тон с насыщенностью из новой палитры. Тогда новая
тема получает иерархию контрастов даром, вместо двухсот значений, подобранных на глаз.

**Три правила, без которых метод ломается:**

1. **Светлоту нельзя сохранять как есть — её надо ОТОБРАЖАТЬ.** Первый заход брал светлоту
   донора буквально: фон редактора вышел на треть светлее фона палитры, а у двух палитр из
   четырёх СОВПАЛ. Причина: палитры одного семейства отличаются друг от друга в первую очередь
   светлотой подложки, и слепое сохранение шкалы донора эту разницу стирает. Лечение — якорная
   карта «ступень донора → ступень палитры» (двадцати записей хватает на две сотни ключей),
   между якорями кусочно-линейно.
2. **Акценты подменяются по смыслу, а не перекрашиваются.** Красный ошибки обязан остаться
   красным в любой теме. Механическому перекрашиванию отдаются только нейтральные подложки
   (порог по насыщенности), остальное — явной картой.
3. **Синтаксис из палитры не выводится.** Тёплое семейство целиком лежит в оранжево-жёлтом
   секторе: primary, link, info и warning отличаются на считаные градусы, и роли подсветки
   слипаются. Роли разводятся **по кругу** с фиксированным тоном за ролью, а светлота и
   насыщенность берутся у темы — тогда четыре темы звучат по-разному, но узнаваемо. Фон
   коричневый, код разноцветный: так устроены все хорошие тёплые темы.

**Проверять числом, а не глазами.** Контраст каждой пары «текст на подложке» сверяется с
донором (дрейф в пределах ±0.5 — норма), различимость ролей подсветки — попарным расстоянием
по RGB. Порог 40 ловит слипание: до разведения по кругу было 11–36, после — 43–52.

---

## [грабля] VS Code кэширует прочитанные темы — правка JSON под работающей IDE не видна

**Симптом:** файл темы изменён, IDE перезагрузила тему, а цвета старые. Смоук показывает
значения предыдущей версии и выглядит как провал правки.

**Причина:** цветовые темы читаются один раз и живут в памяти окна.

**Лечение:** перезапуск IDE. При смоуке тем это обязательный шаг между генерацией файлов и
замером — иначе проверяется вчерашний файл.

**Смежное по смоуку тем:**
- Отладочный порт открывается только с флагом `--cdp` (`run-dev.sh`), и порт по умолчанию
  **9224**, а не 9222.
- CDP-скрипт запускать из корня репозитория: `ws` резолвится от расположения файла, не от cwd.
- Окно поднимает CDP раньше, чем отрисовывается воркбенч; запрос к `.monaco-workbench` в этом
  промежутке повисает без ответа и роняет скрипт на нерешённом `await`. Нужен цикл ожидания.
- В `Input.dispatchKeyEvent` поле `text` только для печатаемых символов; для Enter и прочих
  управляющих его быть не должно, иначе `Invalid 'text' parameter`.
