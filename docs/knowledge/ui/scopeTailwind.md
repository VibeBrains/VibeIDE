...
**Рецидив 2026-07-30 — белый экран вместо неверных стилей.** Копирование бандлов вместо `npm run compile` безопасно, только пока правка целиком внутри `react/src`. Если React-бандл начал импортировать **новый экспорт из TypeScript** (в том случае — `BUILT_IN_PRESETS` из `vibeSubagentRegistryService`), в `out/` этого экспорта ещё нет: workbench падает на импорте и окно остаётся **пустым** — ни `.monaco-workbench`, ни ошибок в `main.log`. Признак: `document.body.innerHTML` есть (≈800 байт каркаса), а workbench-узла нет. Правило: правка задела `.ts` вне `react/src` → только `npm run compile`, копирование бандлов не спасёт.

**Смежная ловушка того же расследования:** CDP-клиент, берущий первый `page`-таргет, может подключиться к **окну DevTools** — соединение живое, `Runtime.evaluate` работает, но каждый workbench-селектор возвращает пусто, и это читается как «окно не отрисовалось». Выбирать таргет по URL (`url.includes('workbench')`), а не по порядку.
