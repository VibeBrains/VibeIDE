...
← [Knowledge Index](../README.md) · связано: [[scopeTailwind]] (`@@`-escape — критично для этого компонента)
...
То есть правило: **инлайн-литерал в `className={...}` → `@@`; класс из переменной → без `@@`.** (Это частный случай общей ловушки из [[scopeTailwind]] «классы только в константах».)
