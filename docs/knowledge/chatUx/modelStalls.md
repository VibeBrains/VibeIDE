...
### #017 — 2026-07-30 — MiniMax-M3 напрямую: «Empty response (reason: stop)» — и выяснилось, что ПРИЧИНУ мы физически не могли увидеть

- **Где:** dev-стенд (`scratchpad/dispatch-smoke`), Agent, провайдер **`minimax` напрямую** (`https://api.minimax.io/v1`, НЕ через агрегатор), модель `MiniMax-M3`, сборка 1.9.1 @ `17a280e8`.
- **Симптом:** ход 3 из смоук-прогона оборвался тостом `VibeIDE: Empty response from minimax/MiniMax-M3 (reason: stop).` Лог: `logs/20260730T202900/vibeide.log:122-123`, `fullError: null`, `rateWaitStreak: 0`.
- **Тайминг (из лога `llmTurn`):** iter1 14.4с (tool_call `ls_dir`), iter2 4.9с (`read_file`, `textLen: 2`), iter3 — 4.6с и пусто. Ответ **пришёл**, это не таймаут и не HTTP-ошибка.
- **Проверка гипотезы «упёрлись в лимит частоты» — ОПРОВЕРГНУТА числами:** три запроса за 24 секунды ≈ **7.5 RPM** при официальном лимите **200 RPM / 10M TPM** для MiniMax-M3 (<https://platform.minimax.io/docs/guides/rate-limits>). До потолка в 25 раз. Гипотеза TPM/RPM тянулась с **#001 (18.05)** и впервые проверена только сейчас.
- **Что при этом НЕ опровергнуто:** у MiniMax поверх RPM/TPM есть **Token/Coding Plan** — отдельная квота со **скользящими 5-часовыми и недельными окнами** (<https://platform.minimax.io/docs/token-plan/intro>). Она бьёт независимо от частоты, и по ней мы данных не имели вовсе.
- **Настоящий корень (диагностический, НАШ):** у MiniMax **два независимых канала ошибок** — HTTP-статус и бизнес-код `base_resp.status_code` в теле (успех = 0), задокументированный прямо в схеме `chat/completions`. Отказ приходит внутри HTTP 200 → для нас неотличим от «модель замолчала». А мы:
  1. `base_resp` не разбирали **нигде** (греп по `src` пустой);
  2. на пути пустого ответа отдавали `fullError: null` (`aiSdkAdapter.ts`) — единственный путь, где нет объекта ошибки, несущего статус/заголовки;
  3. собранный снимок квоты (`lastQuota`) клали в успешный ответ и **выбрасывали в ошибке** — ровно там, где он нужен;
  4. **дефект логики:** авто-пауза по rate-limit была закрыта условием `!emptyMatch` (`chatThreadService.ts`) — то есть для провайдера, маскирующего лимит под пустой ответ, она была **недостижима в принципе**: прогон вставал с «смените модель» вместо того, чтобы переждать окно.
- **Конфликт источников вендора (важно при чтении кодов):** официальная таблица ошибок называет `1002` = rate limit, `1039` = token limit; **собственный CLI MiniMax** (`MiniMax-AI/cli/ERRORS.md`) трактует ту же пару как **фильтр контента**. Плюс CLI знает `1028/1030` (квота) и `2061` (модель не в тарифе), которых в публичной таблице нет. Поэтому классификатор различает коды и **помечает вердикт `ambiguous`**, разводя пару по тексту `status_msg`.
- **Совпадение с чужими наблюдениями (не только у нас, и не только через агрегатор):** [openclaw#1622](https://github.com/openclaw/openclaw/issues/1622) — «ответы стримятся в UI, затем сохраняются пустыми (0 токенов), stopReason: stop»; [MiniMax-M2#109](https://github.com/MiniMax-AI/MiniMax-M2/issues/109) — HTTP 200 + `text/event-stream`, который никогда не завершается. Ответа вендора нет ни там, ни там.
- **Фикс (2026-07-31):** `common/minimaxBaseResp.ts` (чистый парсер + классификатор), `common/providerRequestRate.ts` (наблюдаемый RPM в скользящем окне), перехват тела в `makeCustomFetch` через passthrough-`TransformStream` (не `tee`/`clone` — без back-pressure и без двойной буферизации), поле `diagnostics` в контракте `OnError`, и снятие блокировки авто-паузы при **структурно доказанном** `rate-limit`. Квота (`quota`) намеренно НЕ уходит в ожидание: 5-часовое окно внутри хода не переждать — показывается прямой текст.
- **Честно:** причина конкретно этого обрыва **не установлена** — установлено, что установить её было нечем, и это починено. Следующий такой обрыв обязан предъявить либо `bodyCode`, либо факт его отсутствия.
- **Смоук фикса (2026-07-31, dev-IDE живьём):** happy-path на живом MiniMax-M3 — 7/7 ходов без обрыва (инструментация не мешает); отказный цикл на мок-провайдере (HTTP 200 + `base_resp 1002`, пустой SSE) — лог предъявил `bodyCode:1002, refusalKind:"rate-limit", requestsInWindow:1→2`, авто-пауза прошла все 3 попытки с авто-возобновлением и честно остановилась на потолке. Вся цепочка «тело → диагностика → пауза» доказана прогоном, не только юнитами.
- **Связано:** #001 (тот же симптом, там же впервые записана гипотеза TPM/RPM), #009/#014/#016 (та же семья), см. паттерн «Пустой ответ minimax-семейства» ниже.

---

### #016 — 2026-06-30 — MiniMax-M3: hard-stall auto-retry плодил «осиротевшие» стримы (НАШ баг, чинится)

- **Где:** проект EventHub, Agent, провайдер `minimax`, модель `MiniMax-M3`, сборка VibeIDE 1.5.0. Источник — 7 авто-отчётов диагностики столлов (`.vibe/diagnostics/stall-2026-06-30T*.md`).
- **Симптом (зонтик, провайдер):** MiniMax-M3 хронически медленный/флакающий — десятки `soft-stall noFirstToken` (30с) и `hard-stall` (120с) за сессию, каскады `hard-stall → transport-reset(auto-retry) → hard-stall`. Это провайдерская нестабильность того же класса, что #001/#003/#014, watchdog отрабатывает штатно.
- **Симптом (НАШЕ, в телеметрии):** в таймлайнах видны аномалии, указывающие на баг управления ходом, не на провайдера:
  1. **Два `llmTurn:first-activity` на один `llmTurn:start`** (напр. `first-activity afterMs=1596` → `afterMs=2011`; после reset — `6762` → `26702`).
  2. **`done afterMs` превышает wall-clock и порог 120с** (`afterMs=134455`, `72506`, `70042`, `65653`).
  3. **Перемешанные счётчики `iter`** после `transport-reset` (`iter=3` → `iter=5` → снова `iter=3`) — два цикла итераций крутятся параллельно на одном треде.
- **Корень (подтверждён кодом, `chatThreadService.ts`):** `onHardStall` при зависании делает `abort()` и запускает повтор **отдельным** `_runChatAgent` через 2с-таймер. Два дефекта на «медленном» аборте (ровно сценарий хард-стола):
  1. **Поздние колбэки мёртвого стрима не отсекались.** `messageIsDone` ставился в `true` только в `onFinalMessage`; после `abort` он оставался `false`, поэтому буферизованный поздний `onText` от зависшего стрима проходил guard и писал фантомный `first-activity`, портил retry-кэш, перетирал UI новой попытки.
  2. **Старый цикл клобберил новый прогон.** Цикл, припаркованный на `await messageIsDonePromise`, просыпался уже после того, как detached-повтор выставил `isRunning='LLM'` → guard `isRunning!=='LLM'` (≈6316) его не ловил → он повторно обрабатывал ход / затирал состояние нового прогона или баннер ошибки.
  3. **`firstTokenReceived` объявлен пер-ран, а не пер-стрим** → на повторной итерации после попытки, получившей токены, новый стрим не писал свой `first-activity` и репортил `anyToken=true` в хард-столе, даже когда токенов не было (источник аномалий #1/#2 в трассе).
- **Фикс (реализован 2026-06-30, `chatThreadService.ts`):**
  1. В `onHardStall` сразу после `abort()` — `messageIsDone = true` (поздние колбэки инертны) + новый пер-итерационный флаг `hardStallHandled = true`.
  2. После guard'а `isRunning!=='LLM'` — `if (hardStallHandled) { return; }`: припаркованный старый цикл выходит, чем бы стрим ни резолвился (aborted/done/error), ничего не трогая.
  3. `firstTokenReceived = false` в начале каждой итерации (рядом с `_turnStartMs`). Безопасно: `markFirstToken`/`markNetworkEnd` в `chatLatencyAudit` идемпотентны (первая попытка выигрывает), TTFS-метрики не искажаются.
  - Detached-таймер повтора и terminal-error ветка оставлены как есть — сохраняется устойчивость к «никогда не резолвящемуся» аборту (UI не виснет). Контрол-флоу не перестраивался.
- **НЕ делали (осознанно, YAGNI):** backoff/расширение лимита авто-ретрая (`HARD_STALL_AUTO_RETRY_MAX` уже =1, reset на успехе — реального churn'а нет) и policy-ветвление по `anyToken=false/true` — выгода спорная, риск выше. См. урок #005 (не плодить предохранители спекулятивно).
- **Честно:** зонтик инцидента — медленность/флак MiniMax-M3 (в нашем коде по самим столлам чинить нечего), но три дефекта выше — НАШИ, воспроизводимы на любом провайдере с «медленным» абортом, и портили как UI, так и саму диагностику.
- **Связано:** #001/#003/#014 (та же провайдерская нестабильность minimax), #008/#010 (другие баги управления ходом в `chatThreadService.ts`), #005 (тот же принцип — лечить причину, не плодить счётчики).
...
- **Round-2 (2026-05-30) — фикс #013.2 промахнулся на маршаленной ошибке.** Спам `InitializeModel error: EntryNotFound (FileSystemError)` повторился (проект BookCatalog, minimax через openCode, файл `…/.vibe/rules/dev.md` при создании). Корень: фикс 2026-05-28 распознавал raw `FileSystemProviderError` через `instanceof`, но ошибка прилетела из **MAIN-процесса через IPC** (стек в `main.js`) — при маршалинге теряется прототип, `instanceof` (обоих классов) = false → ENOENT снова не распознан → ре-throw → кэш существования не заполнен → лог на каждом тике. **Фикс:** добавлена prototype-независимая ветка `toFileSystemProviderErrorCode(err) === FileNotFound` — она читает код с `error.name` (`"EntryNotFound (FileSystemError)"`), не завися от `instanceof`. **Урок (footgun):** для ошибок, пересекающих process/bundle-границу, `instanceof <ErrorClass>` ненадёжен — детектить по `toFileSystemProviderErrorCode`/коду/имени, не по прототипу.
...
- **Фикс (реализован 2026-05-28):**
  ...
  - Детали механики — `docs/knowledge/chatUx/circuitBreakers.md`.
...
### Паттерн: пустой ответ / зависший стрим у minimax-семейства

- **Симптомы:** ход завершается без текста, без reasoning и без tool-call. В UI — `Empty response from <provider>/<model> (reason: stop|unknown)` либо watchdog `Stream stalled` через 120с. HTTP-ошибки нет, ответ формально успешен.
- **Условия проявления:** модели `minimax-m2.x` / `MiniMax-M3` — **и через агрегатор (openCode/zen), и напрямую** через `api.minimax.io`. Чаще на длинных tool-only сериях (8-12+ ходов подряд без текста между ними), но воспроизводилось и на третьем ходу. Размер промпта роли не играет: наблюдалось при 6-13% заполнения контекста.
- **Инциденты в корзине:** #001 (18.05, агрегатор), #009 (25.05, агрегатор), #014 (28.05, stall), #016 (30.06, hard-stall каскад), #017 (30.07, **прямое подключение**).
- **Что это НЕ:** не переполнение контекста (проверено долей заполнения), не наш парсер tool_calls (починен в #001 → миграция на AI SDK), не размер system-промпта. Прямое подключение в #017 сняло и версию «виноват агрегатор».
- **Гипотеза о причине (частично проверена):**
  - **Опровергнуто:** «упёрлись в RPM» — в #017 фактическая частота была ~7.5 RPM при лимите 200.
  - **Открыто:** квота Token/Coding Plan (скользящие 5-часовые и недельные окна) и отказ, переданный в `base_resp` внутри HTTP 200.
  - **Открыто:** нестабильность самого стрима у вендора — подтверждается чужими баг-репортами на официальном API без ответа вендора.
- **Известные митигации:**
  - **Диагностика (с 31.07):** любой такой обрыв теперь несёт `diagnostics` — HTTP-статус, заголовки лимитов, `base_resp` (`bodyCode`/`bodyMessage`) и **фактический RPM в минутном окне**. В логе — строка `[VibeIDE/chatThread] onError` со всеми полями. **Прежде чем строить гипотезу — прочитать её.**
  - Доказанный `rate-limit` в теле → авто-пауза и продолжение сама (раньше была недостижима).
  - Доказанная `quota` → ожидание не предлагается: окно длиннее хода.
  - Брейкер пустых ответов (порог `vibeide.chat.emptyResponseCircuitBreakerThreshold`, дефолт 3) — на случай, когда тело молчит.
  - Для ответственной работы — стабильная модель (deepseek/claude), вывод #004 в силе.
- **Связанные инциденты:** #001, #009, #014, #016, #017.

> Следующий шаг для этой корзины: собрать 2-3 обрыва **с диагностикой** и закрыть вопрос «квота или нестабильность стрима» фактом `bodyCode`.
...
## Номер попытки в таймлайне: чей таймер выстрелил (08.08.2026)

Разбор трёх диагностик с рабочей машины (Promed, сборка 1.10.0) вскрыл не дефект кода, а **дефект читаемости отчёта**. В таймлайне встречалось:

```
14:59:03 llmTurn:start
14:59:04 llmTurn:hard-stall sec=120, anyToken=false
```

Это правда: hard-stall принадлежал попытке, начатой двумя минутами раньше (перед этим шли три `llmTurn:start` подряд без `done` — пользователь жал повтор, попытки наложились). Но по отчёту это неотличимо от «запрос, стартовавший секунду назад, завис на 120 с» — **верный лог, из которого уверенно делается неверный вывод**.

Теперь каждое событие хода несёт `turn=<N>` (монотонный счётчик процесса, не потока: две живые попытки не должны получить один номер), а рендер помечает события, чей `turn` меньше последнего стартовавшего, как «⟵ от прошлой попытки». События без `turn` (старые сборки, трассы инструментов) не помечаются — догадка о них выдумала бы факт.

Рендер вынесен в чистый `common/chatTraceRender.ts` (3 теста) и используется и командой «Показать трейс прогона чата», и отчётом о зависании.

**Из тех же диагностик, для истории:** молчание `minimax-anthropic` (три `start` без единого `first-activity`) и ровные 30-секундные отказы `zai` — это баг `baseURL` без `/v1` и подписочный ключ на пер-токенном эндпоинте, исправленные `369ff85c` и вошедшие в v1.11.0. А вот `MiniMax-M3` действительно нестабилен на первом токене (48 с / 36 с / 10 с против 2–5 с у `M2.7-highspeed` в том же прогоне) и деградирует с ростом переписки — это к вендору, не к нам.
