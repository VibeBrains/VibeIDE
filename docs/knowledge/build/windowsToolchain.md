...
## [баг] Пустой submodule `.vibe-defaults` → сборка молча выпускает IDE без дефолтов агента

**Контекст:** Windows-догон 1.16.0 (2026-09-02). Артефакт был собран, опубликован — и только потом выяснилось, что он неполон.

**Суть:** `.vibe-defaults/` — git submodule (общий набор сидов с VibeIDEA). На чекауте, где его не инициализировали, папка существует, но пуста. Генератор `gen:vibe-defaults` не считает это ошибкой: он честно строит манифест из того, что видит, и выдаёт `VIBE_DEFAULTS_MANIFEST = []`. Дальше **всё зелёное** — компиляция 0 ошибок, упаковка, смоук собранного приложения, публикация. В установщике манифест весит 709 байт вместо 458 КБ, и у пользователя не засеваются ни провайдеры, ни промпты, ни правила, ни дизайн-контекст.

**Как обнаружено:** случайно, по `git status` после сборки — регенерированный `vibeDefaultsManifest.generated.ts` показал −133 строки. Ни один гейт проекта дефект не видел.

**Проверка факта:** `wc -c out-build/vs/workbench/contrib/vibeide/common/vibeDefaultsManifest.generated.js` — 709 байт у битой сборки, ~458 КБ у здоровой; `git submodule status` показывает `-<hash>` (минус = не инициализирован).

**Лечение:** `git submodule update --init --recursive` → `npm run gen:all` (манифест обязан совпасть с версией в git) → полная пересборка → перезалив `gh release upload --clobber` → пересоздание winget-PR (сумма установщика изменилась).

**Защита:** оба релиз-скрипта перед `gen:all` считают файлы в `.vibe-defaults/` и падают на нуле с инструкцией. Добавлено 2026-09-02.

**Продолжение (2026-09-10, релиз 1.20.0): счётчика файлов НЕДОСТАТОЧНО.** У устаревшего чекаута правильное количество файлов и неправильное содержимое: сиды живут в чужом репозитории, и подмодуль, оставленный на старом коммите, засевает набор прошлых релизов, пока любая проверка «по количеству» довольна. Поймано пре-флайтом: `git submodule status` показал `+` — дерево на `c1ac2a53` (69 файлов), а тег `v1.20.0` закрепляет `f1903c5c` (76 файлов, десять коммитов разницы: `products.json`, рабочие конфиги вместо примеров, новые провайдеры). Прошлый дефект был «пусто вместо данных» и бросался в глаза; этот — «старое вместо нового», тише на порядок. **Теперь оба скрипта сверяют указатель:** `git ls-files -s .vibe-defaults` против `git -C .vibe-defaults rev-parse HEAD`, при расхождении — остановка с обоими хешами. Мера того, что это не косметика: манифест в сборке 458 084 → 506 454 байта после обновления подмодуля.

**Пре-флайт догона (30 секунд, делать всегда):** `git submodule status` — префикс `+` или `-` означает «стоп»; `git ls-tree vX.Y.Z .vibe-defaults` против `git ls-files -s .vibe-defaults`; `find .vibe-defaults -type f | wc -l`.

**Это происходит КАЖДЫЙ релиз, а не изредка (подтверждено 1.20.0 и 1.21.0 подряд).** Сиды живут в VibeBrains и развиваются между нашими релизами, а `git pull` подмодуль не трогает — он лишь двигает *указатель* в индексе, оставляя рабочее дерево на прежнем коммите. Масштаб расхождения за четыре дня: 1.20.0 — 69 → 76 файлов, 1.21.0 — 76 → 83. Поэтому **`git submodule update --init --recursive` — обязательный первый шаг догона**, а не проверка на всякий случай; гейт в релиз-скриптах страхует от забывчивости, но полагаться на падение сборки вместо тридцати секунд пре-флайта — расточительно.

**Применение:** любой свежий клон и любая машина, где догоняют платформу; это же относится к CI. Класс дефекта — «тихая деградация»: тот же, что исполняемый бит CLI в бандле и молчащий смоук.
...
> ⚠️ **С версии 1.15.1 требуемая нода — `24.18.0`** (обновление базы до VS Code 1.133). Ниже описан
> случай 1.2.2, когда пином была `22.22.1`, а Node 24 ронял упаковку. **Сегодня всё наоборот:**
> `build/npm/preinstall.ts` требует мажор ровно из `.nvmrc`, и на Node 22 установка не начнётся.
> Механика граблей осталась прежней — важно совпадение с `.nvmrc`, а не конкретная цифра.

**Суть:** TS-компиляция проходит на **любой** ноде, поэтому ошибка обманывает — билд доходит до упаковки и только там умирает. Корень: **`gulp-atom-electron`** в `package-win32-x64` ломается на неверной **мажорной** версии Node и глотает ошибку стрима. Требуемая версия пинится в **`.nvmrc`**; сборка на чужом мажоре даёт именно этот тихий фейл. Поднимать TS-компиляцию мало — упаковке нужна правильная нода.

**Управление версиями — через `fnm`** (не nvm-windows: тот завязан на симлинк `C:\Program Files\nodejs` и требует сноса standalone). `winget install Schniz.fnm`; `fnm install 24.18.0` (версия из `.nvmrc`; на релизах до 1.15.0 — `22.22.1`).
...
- **Для скриптовой сборки надёжнее всего префиксовать PATH каталогом установки**, а не полагаться на `fnm env`-симлинк (в неинтерактивном шелле капризничает):
  `$node = Split-Path (fnm exec --using=24.18.0 node -e "process.stdout.write(process.execPath)"); $env:Path = "$node;$env:Path"` → дальше `node`/`npm.cmd`/gulp резолвятся в нужную версию (путь вида `…\AppData\Roaming\fnm\node-versions\v24.18.0\installation`).
...
## [баг] Фаза 1 ломает Фазу 2: смоук-чек оставляет живые `VibeIDE.exe`, и `clean` падает с `EBUSY`

**Контекст:** Windows-догон v1.10.0 (2026-07-31). Фаза 1 прошла с exit 0, Фаза 2 (`-SkipCompile`) сразу же упала:
`Error: EBUSY: resource busy or locked, rmdir 'D:\Projects\VibeCode\VibeIDE-win32-x64'` на таске `clean-vibeide-win32-x64`.

**Суть:** в конце сборки скрипт делает смоук-чек `VibeIDE-win32-x64\VibeIDE.exe --version` и печатает
`Smoke check WARNING: ... produced no output (non-fatal)`. Пометка **«non-fatal» вводит в заблуждение**: Electron-приложение не отдаёт `--version` в stdout при таком запуске, но при этом **реально стартует как GUI и остаётся жить** — после Фазы 1 в системе висело **8** процессов `VibeIDE.exe` с временем старта ровно в минуту смоука. Они держат каталог `VibeIDE-win32-x64`, который Фаза 2 чистит первым шагом переупаковки → `EBUSY`. То есть варнинг не безобиден: он **систематически** роняет следующую фазу, а не «иногда».

**Обход (сейчас):** между фазами снять процессы и повторить Фазу 2 — она отработает штатно:
`taskkill /F /IM VibeIDE.exe`
Проверять фактом до запуска Фазы 2: `Get-Process -Name VibeIDE | Where-Object { $_.Path -like "*VibeIDE-win32-x64*" }`. Смоук Фазы 2 плодит зомби **повторно** — чистить и после неё.

**Причина, а не заплатка (долг):** чинить сам смоук-чек в `scripts/release-windows.ps1` — запускать с таймаутом и гарантированно завершать порождённый процесс (`Wait-Process -Timeout` + `Stop-Process`), либо проверять сборку способом, не поднимающим GUI. Пока не сделано — обход выше обязателен.

**Применение:** любой прогон Фаза 1 → Фаза 2 на Windows; диагностика `EBUSY` на `clean-vibeide-win32-x64`. Родственный `EBUSY` по `node_modules` — запись про MSB8040 выше.
