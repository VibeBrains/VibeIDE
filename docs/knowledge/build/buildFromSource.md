...
3. **Node** — версия из `.nvmrc` (сейчас `22.22.1`): `fnm install`, затем каталог версии префиксуется в `PATH` (на Windows `fnm exec --using npm` ломается — резолвим путь и добавляем явно, см. [windowsToolchain.md](windowsToolchain.md)).
