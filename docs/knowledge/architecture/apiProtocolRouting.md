...
## [правило] Поле спеки ищется в примере ответа, а не по памяти (2026-09-19)

**Контекст:** 18.09 мы научились перечитывать список инструментов MCP по сроку годности и читали
`ttlMs` из `_meta`. Ревизия 2026-07-28 кладёт `ttlMs` и `cacheScope` в КОРЕНЬ результата, рядом с
`tools`. Код компилировался, тесты проходили (на наших же выдуманных снимках), а против настоящего
сервера перечитка не завелась бы ни разу.

**Суть:**
- Новое поле спеки проверяется по ПРИМЕРУ ответа на странице метода, а не по тексту раздела:
  раздел говорит, что поле есть, пример — где оно лежит
- Тест на выдуманном снимке подтверждает только то, что разбор согласован сам с собой. Снимок для
  теста берётся из примера спеки дословно
- Перечисления сужаются до объявленных значений: `cacheScope` — ровно `public` или `private`, а
  «своё слово сервера» было нашей выдумкой

**Прецедент рядом (тот же день):** две темы дайджеста оказались легендами, которые опровергаются
одной страницей вендора — «Kimi K3 принимает только `max`» (это умолчание, уровни работают) и
«у MiniMax для подписки свой путь `/anthropic`» (адрес один, различается префикс ключа). Правило то
же: утверждение о чужом API проверяется на странице этого API, а не в чужом конфиге и не в
баг-репорте, где автор сам пишет «may have subtle incompatibilities».
