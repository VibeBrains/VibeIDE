...
## [дефект] 2026-09-25 — системный промпт терялся у всех моделей, кроме встроенного Anthropic

**Симптом:** не наблюдался — модель отвечает и вызывает инструменты. Найден чтением кода при разведке стабильного префикса
и доказан тестом провода: маркер из системного сообщения не дошёл ни до одного из четырёх проводов.

**Причина:** конвертер (`convertToLLMMessageService.ts`, размещение по `supportsSystemMessage`) кладёт промпт в список
сообщений с ролью `system` (`system-role` — умолчание для всех моделей) или `developer` (OpenAI). В `separated`, то есть
в отдельное поле, он уходит только у встроенного Anthropic. Адаптер AI SDK (`convertMessagesToModelMessages`) роли
`system`/`developer` выбрасывал в расчёте на `separateSystemMessage` — а оно у таких моделей пустое. Отладочная секция
`chatThreadService` дописывалась в пустое `separateSystemMessage` и уходила ВМЕСТО промпта.

**Решение:** `systemPromptOf` — текст сообщений `system`/`developer` плюс `separateSystemMessage` становится системным
промптом вызова, дальше провод решает AI SDK (у reasoning-моделей OpenAI — `developer` сам SDK).

**Урок:** конвертация, которая выбрасывает часть входа «потому что это возьмут в другом месте», должна проверяться тестом
на то самое другое место. Тесты адаптера всегда передавали `separateSystemMessage` и пути через список не видели.
