...
3. **При добавлении нового провайдера (см. [[aiSdkMigration]] — чат у всех идёт через AI SDK)** — расширить `detectModelFamily` switch-кейсом по providerName.
...
**Почему read открыт по умолчанию:** жёсткий запрет тривиально обходился через `run_command` + `Get-Content`, поэтому давал не безопасность, а трение (агент застревал в обходных попытках, тегами протекая в чат — см. incident в [xmlToolNormalization.md](./xmlToolNormalization.md) Layer 4). Write остаётся закрытым: защита от случайной записи в системные файлы и соседние проекты.
...
- [aiSdkMigration.md](aiSdkMigration.md) — завершённая миграция провайдеров.
