...
- **Ноль — это не «неизвестно», это «бесплатно».** Ни в профиле, ни в дефолте не оставлять `cost: {0,0}` как заглушку: приблизительная цена похожей модели честнее, потому что не врёт роутеру в другую сторону. Заглушку `TODO: Verify pricing` считать багом, а не пометкой.
