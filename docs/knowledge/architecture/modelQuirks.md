...
- **`reasoning_effort` — РАСХОЖДЕНИЕ: три уровня `low`/`high`/`max` (default `max`), а не единственный `max`.** `platform.kimi.ai` + techtimes (2026-07-18) описывают три тира усилия (смена уровня инвалидирует context cache). Это противоречит фиксу 23.07 (`["max"]`) и премиссе пробы A выше. **Не переписывать как факт** — источник свежий/вторичный по части «три уровня»; проба A из смоука меняет смысл: теперь она проверяет не «есть ли `high`», а «сколько уровней реально принимает API». Подтвердится — вернуть `effort.values` = `["low","high","max"]` (default `max`), `canTurnOff` оставить `false` (thinking не отключается). thinking всегда включён — это подтверждено обоими источниками.
...
**Суть:** Google объявил три классических sampling-параметра устаревшими — «deprecated and ignored. In future model generations, supplying these parameters returns an HTTP 400 error» ([changelog](https://ai.google.dev/gemini-api/docs/changelog), [latest-model](https://ai.google.dev/gemini-api/docs/latest-model)). Взамен глубина управляется строковым `thinking_level` (`minimal`/`low`/`medium`/`high`; у 3.6 Flash дефолт `medium`, у 3.5 Flash-Lite — `minimal`), а не числовым `thinkingBudget`.

Нашего **нативного** пути это не касается: `sendGeminiChat` кладёт в запрос только `systemInstruction`, `thinkingConfig` и `tools` ([`sendLLMMessage.impl.ts:1594-1600`](../../../src/vs/workbench/contrib/vibeide/electron-main/llmMessage/sendLLMMessage.impl.ts#L1594)) — sampling туда не попадает вовсе. Правило работает только на агрегаторном пути, поэтому сегодня оно безвредно (параметры молча игнорируются), но на следующем поколении моделей превратится в 400. Отдельно: `thinkingConfig` мы формируем как `{ thinkingBudget }` и только для `type === 'budget_slider_value'`, а у профилей `gemini-3-*` стоит `reasoningCapabilities: false` — до Gemini 3 слайдер не доходит, модель всегда идёт на вендорском дефолте.

**Применение:** правило `match: "gemini"` сузить до поколений, которые эти параметры ещё принимают (`gemini-2`, `gemini-1.5`), а не держать широким. Общий урок для каталога: **широкий `match` по имени семейства живёт дольше, чем поведение, ради которого он написан** — вендор меняет контракт внутри семейства, а подстрока продолжает матчиться. Правило, завязанное на поколение, и матчить надо по поколению.

**Не проверено:** поведение конкретного агрегатора (OpenRouter и пр.) на `gemini-3.6-flash` с sampling-параметрами живьём не гонялось — вывод сделан из вендорской доки и чтения нашего кода.
