...
**Третий пробел — настоящий гэп каталога.** Сила размышления у Glimmer задаётся строкой
`Reasoning strength: low|medium|high|xhigh` в **системном промпте**, а не полем
`reasoning_effort`. Наши поля квирков (`forceEmptyReasoning`, `forceToolCallFormat`,
`mirrorReasoningContent`, `forcedToolChoiceUnsupported`, сэмплинг) такого не умеют, поэтому
слайдер усилия на Glimmer молчит. Заведено в roadmap отдельным пунктом.
