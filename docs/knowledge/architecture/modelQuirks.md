...
## [квирки] Длинное правило старой модели перекрывает новую модель семейства (2026-09-17)

**Контекст:** слияние правил отдаёт приоритет более длинному `match`.
`kimi-k2.7-code` содержит подстроку `kimi-k2`, поэтому получил правило устаревшей модели без рассуждения:
temperature 0.6 и никакого возврата `reasoning_content`.
Общий `kimi` (1.0 / 0.95) короче и проиграл.
Вендор закрепил у K2.7 Code temperature 1.0 и top_p 0.95 — другое значение даёт ошибку, а без возврата рассуждения
в раундах инструментов со второго хода приходит 400.

**Суть:**
- Префиксное правило старой модели (`kimi-k2`) — ловушка для всех будущих id с тем же префиксом (`kimi-k2.7`, `kimi-k2.8`…)
- Новая модель семейства требует явного правила или проверки, что её id разбирается в нужные причуды
- Проверка — фактом: `matchQuirks(rules, id)` из `out/` на id из примеров набора, а не чтением каталога глазами

**Применение:** добавляя модель в пример провайдера, дописать её id в `test/node/modelQuirksCatalog.test.ts`
с ожидаемыми причудами — тест падает, если id съело правило другой модели.
...
## Широкий `match` тихо перебивает явный `toolFormat` (2026-08-12)

Матчер каталога — голая подстрока по id модели без учёта регистра, а квирки в
`aiSdkAdapter.sendViaAISdk` стоят **первым ярусом** и бьют возможности провайдера безусловно.
Значит правило, написанное под одно поколение, дотягивается до каждой будущей модели, в id
которой окажутся те же буквы.

Так и вышло: `match: "qwen"` с `forceToolCallFormat: "xml"` завели под поколение 2.5 на прямом
API, а оно молча увело в XML-грамматику `qwen3.7-plus` и `qwen3-coder-next` — обе объявлены в
**нашем же** пресете `alibaba-coding-plan.jsonc` с `toolFormat: "openai"`. Ничего не падало:
модели просто работали хуже, чем могли.

Лечится сужением `match` до поколения, которому форс действительно нужен, — не ослаблением
проверки. Сам форс для 2.5 сохранён отдельным правилом; причина, ради которой он вводился
(путаница имён параметров, `path_pattern` вместо `pattern`), с 0.13.18 закрыта общим
arg-repair — по этой же причине openCode-правило для qwen давно вернули в native.

**Гейт:** `npm run quirks-conflicts-check` (`scripts/vibe-quirks-conflicts.mjs`, отдельный
workflow) — ни одно правило каталога не смеет форсить формат инструментов модели, для которой
поставляемый пресет объявил `toolFormat`. Ни компилятор (два JSON-файла), ни юнит-тест матчера
(он проверяет свои же правила, а не парк реальных id) этого класса не видят. Проверен обеими
сторонами: на возвращённом широком `qwen` краснеет ровно на двух моделях, после сужения зелёный.

## Сюита «e2e по бандлу» проверяла копию, а не каталог (2026-08-12)

`modelQuirksService.test.ts` называлась «end-to-end integration via bundled rules», но список
правил в ней был **переписан руками** с пометкой «kept in sync with the JSON catalog». Это ровно
тот второй источник правды, ради устранения которого существует `bundledCatalog.ts` (он грузит
`resources/model-quirks.json` импортом с атрибутом, «чтобы дрейф был невозможен по построению»).

Копия закономерно разошлась: правки каталога уходили в релиз, а сюита оставалась зелёной, потому
что проверяла вчерашний слепок. Зелёный тест читался как «правило проверено», хотя не проверял
ничего. Теперь сюита зовёт `loadBundledCatalog()` и прогоняет `validateCatalog()` — тот же путь,
которым каталог грузит продукт.

Признак, по которому это ловится в чужом коде: комментарий вида «держится в синхроне с …».
Синхрон, который держит человек, уже разошёлся — вопрос только когда это заметят.

## [квирки] Усилитель уровня складывается вниз, а не вверх (2026-09-18)

**Контекст:** модель принимает не все уровни мышления, и сохранённый уровень приводится к ближайшему.
Правило «при ничьей побеждает верхний» отправляло `xhigh` в `max`.

**Суть:**
- Таблица DeepSeek (api-docs.deepseek.com/guides/thinking_mode, сверено 18.09.2026): `minimal→low`,
  `medium→high`, `xhigh→high`, `ultra→max`. То есть у вендора нет единого «вверх» или «вниз»:
  `medium` идёт вверх, `xhigh` — вниз
- Объяснение в одну строку: `xhigh` — это «high, но сильнее», `ultra` — «max, но сильнее». Усилитель
  складывается в тот уровень, который усиливает; остальное решается расстоянием
- Цена прежнего поведения денежная: пользователь с сохранённым `xhigh` молча платил по верхнему уровню

**Связано:** `common/reasoningEffortLevel.ts`, тест `reasoningEffortLevel.test.ts`.

## [решение] Мышление Claude 5 задаётся уровнем, и старый формат отвергается (2026-09-18)

**Контекст:** `thinking: {type: "enabled", budget_tokens}` — то, как мы просили мышление у Anthropic.

**Суть:**
- С Opus 4.7 и во всей линейке 5 этот формат отвечает 400; вендор описывает переход парой полей:
  `thinking: {type: "adaptive"}` и `output_config: {effort}`
  (platform.claude.com/docs/en/build-with-claude/extended-thinking, сверено 18.09.2026)
- Записи `claude-opus-5` и `claude-sonnet-5` заведены с окном 1M и ценами со страницы вендора того же дня
- **Грабли сопоставления имён:** ветка `claude-opus-4` совпадает и с `claude-opus-4-7`, а в цепочке
  побеждает последнее присваивание — новую ветку пришлось поставить последней. Поймал тест, а не чтение
- Поведение меняется не только синтаксисом: при адаптивном режиме модель сама решает, думать ли,
  и на низком уровне может не думать вовсе

**Живьём не проверено:** ключа Anthropic нет.

## [грабли] Рассуждение Claude привязано к разговору, а наш системный промпт — нет (2026-09-23)

**Факт вендора.** У Claude Opus 5.5 и Fable 5.1 блок рассуждения действителен только при том префиксе, при котором он
родился: `system`, `tools` и все прошлые сообщения байт в байт. У аккаунтов, созданных с 31.08.2026, повтор блока после
правки префикса — 400 (platform.claude.com/docs/en/models/opus-5-5/migration-guide). Старые аккаунты включают проверку
сами. Вместо ошибки можно попросить отбросить недействительный блок: `thinking.block_binding.prefix_mismatch_behavior:
"drop_block"`, бета `thinking-binding-controls-2026-08-01`.

**У нас.** Путь Anthropic (`sendAnthropicChat`) возвращает блоки рассуждения всех прошлых ходов
(`prepareMessages_anthropic_tools`), а системный промпт (`chat_systemMessage`) собирается на каждый запрос и несёт
изменчивое: активный файл, открытые файлы, дерево каталогов, терминалы, дату, правила проекта — последние теперь
перечитываются на лету. Итог для нового аккаунта: ход после смены активного файла падает.

**Что делать.** Быстро — `drop_block` у моделей с привязанным рассуждением. Правильно — стабильный префикс: изменчивое
...
gpt-6-sol и gpt-6-luna на developers.openai.com. Текст ошибки из живого отчёта:
«Function tools with reasoning_effort are not supported for gpt-6-sol in /v1/chat/completions. To use function tools,
use /v1/responses or set reasoning_effort to 'none'».

**У нас.** Транспорт Responses есть (`@ai-sdk/openai#responses`), и файл провайдера объявляет его полем `protocol` у
модели. Но диспетчер (`electron-main/llmMessage/sendLLMMessage.ts`) сначала ищет встроенного провайдера в своей
таблице, и встроенный `openAI` всегда уходит в `_sendOpenAICompatibleChat`, то есть в Chat Completions. Протокол модели
читается только у провайдеров из файлов с собственным `baseURL`. GPT-6 в каталоге возможностей нет, `reasoning_effort`
не уходит, сервер берёт `medium` — и первый же ход с инструментами получает 400.
...
## [дефект→решение] Неподписанное рассуждение не возвращалось на проводе Anthropic (2026-09-25)

**Контекст:** Kimi, MiMo и DeepSeek отдают блоки `thinking` и на своих `/v1/messages`, но без подписи.
По документации MiMo без возврата рассуждения второй раунд инструментов отвечает 400.
Флаг `mirrorReasoningContent` работал только на OpenAI-проводе (`reasoning_content`).

**Суть:** блок терялся трижды подряд, и починка любого одного звена ничего бы не дала.
Сборщик потока (`AnthropicReasoningCollector`) хранил только подписанные блоки.
Преобразователь истории отдавал блоки одному провайдеру с именем `anthropic`, а провайдеру из файла на том же проводе — нет.
`@ai-sdk/anthropic` отбрасывает часть `reasoning`, у которой нет ни подписи, ни скрытых данных, с предупреждением
«unsupported reasoning metadata».

Теперь сборщик хранит неподписанный блок с текстом, а преобразователь пропускает блоки любому маршруту на проводе Anthropic.
Кому что уходит, решает адаптер по правилу VibeIDEA (`replaysThinkingBlock`): модели с `mirrorReasoningContent`
получают все блоки, Claude — только подписанные, остальные — ничего.
Для Claude неподписанный блок — оборванный поток, повтор был бы 400.
Через SDK неподписанный блок проходит с пустой подписью: без неё SDK его выбросит, и потому `makeCustomFetch` вырезает
пустое поле уже из готового тела запроса.
На провод блок уходит без поля `signature`, как у VibeIDEA (`ThinkingBlock.toWire`).

**Применение:** проверка «дошло ли до модели» — только по телу запроса: `test/node/aiSdkAdapter.test.ts` пишет его заглушкой.
Каталог квирков в тестах адаптера не загружен; нужное правило подставляет `__setCatalogForTests`.

**Найдено живым смоуком того же дня.** Тест провода проверял историю в форме блоков Anthropic, а в IDE она приходила
в форме chat completions. Форму выбирает профиль семейства модели: у Kimi это `openai-style` при любом протоколе провайдера.
Рассуждение тогда едет строкой `reasoning_content`, адаптер делал из неё часть без подписи, и SDK выбрасывал её молча.
Теперь на проводе Anthropic строковое рассуждение для модели с возвратом уходит так же, как блок: неподписанным.
Если у сообщения есть блоки `thinking`, строковая копия не добавляется.
