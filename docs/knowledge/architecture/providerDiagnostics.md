...
← [Knowledge Index](../README.md) · связано: [dynamicProviders.md](dynamicProviders.md), [commandsPaletteModal.md](commandsPaletteModal.md), [llmAndContext.md](llmAndContext.md)
...
- UI — отдельный resizable-React-апп (`react/src/provider-diagnostics-tsx`), новая tsup-точка + ручной `.d.ts`, `@@`-className-футган и `.vibe-scroll` — по образцу `commandsPaletteModal.md`.

---

## [решение] «Нет средств» — терминальный отказ, а не rate limit (06.08.2026)

**Симптом:** Z.AI отвечает `HTTP 429, code 1113 — Insufficient balance or no resource package`. Транспорт говорит «слишком часто», поэтому AI SDK честно отрабатывал пять backoff-ов: **шесть попыток за минуту** ради ответа, который не мог измениться. Пользователь при этом читал «Rate limit exceeded» и шёл проверять частоту запросов вместо тарифа.

**Что сделано:** в кастомном fetch (`aiSdkAdapter`) на 4xx читается тело и проверяется чистой функцией `common/providerFundsRefusal.ts`. При «нет средств» ответ подменяется на **402 Payment Required** — SDK ретраит 429 и не ретраит 402. Приём не новый: рядом тем же способом гасится 429 со слишком далёким `retry-after`.

**Замер до/после на живом ключе:** попыток `AI_APICallError` — **172 → 4**, `maxRetriesExceeded` — **18 → 0**, ответ пользователю приходит за секунды вместо минуты.

**Три решения, каждое неочевидное:**

1. **Список фраз намеренно узкий.** Одно слово «quota» в него не входит: месячная квота сама сбрасывается и ближе к rate limit, а retry там осмыслен. На список попадает только то, что остаётся правдой, пока не заплатят или не исправят адрес.
2. **Статус проверяется свободно (любой 4xx).** Вендоры прячут это за 429, 402 и даже 400 — надёжный сигнал даёт формулировка, а не номер.
3. **`statusText` только ASCII.** Это ByteString: не-Latin-1 символ роняет сам конструктор `Response` (уже обжигались em-dash'ем в соседней ветке). Русское объяснение добавляется позже, в карточке чата.

**Грабля, пойманная живым прогоном:** первая редакция клонировала ответ ПОСЛЕ того, как сборщик диагностики начинал читать тело, — `Response.clone: Body has already been consumed`, и вместо отказа провайдера пользователь видел нашу внутреннюю ошибку. Клон делается сразу после получения ответа, до любых чтений. Юнит-тесты этого не видели: они проверяли чистую функцию, а сломался порядок работы с потоком.
