...
## [решение] Кто ответил на самом деле (2026-09-12)

**Контекст:** между нами и вендором стоят прокси, агрегатор и собственная запасная цель, а подмена
модели молчалива: ответ просто приходит из другого места, тогда как счёт считается по запрошенной.

**Суть:**
- Провод называет модель сам: OpenAI-совместимые — полем `model` в каждом чанке, Anthropic — один раз
  в `message_start`, Gemini — `modelVersion`. Имя снимается с головы ответа (`modelEcho.ts`), поток при
  этом не трогается: наблюдатель зеркалит байты и молча сдаётся на любой ошибке.
- Правило сравнения перенесено из VibeIDEA дословно вместе с вектором: префикс агрегатора и
  датированная сборка — не подмена, хвост-слово (`gpt-4o-mini` вместо `gpt-4o`) — подмена. Один и тот
  же ответ обязан читаться одинаково в обоих продуктах, иначе правило разъедется молча.
- Голова ответа обрывается на середине объекта, поэтому после построчного разбора JSON есть запасной
  скан поля: прочитать имя важнее, чем прочитать его красиво.
- В ленте строка один раз на пару «просили → ответила» за окно, в журнале — каждый случай: в ленте
  нужен заголовок, в журнале история.
