...
## [смоук 06.08.2026] Оба конфиг-провайдера «не работали» — причины разные и обе не в коде провайдеров

Живой прогон на ключах владельца (проект VibeShot, `zai` и `minimax-anthropic`). Механика конфига оказалась исправной: файл прочитан, оба провайдера подняты (`providers loaded: 2 provider(s), 0 warning(s)`), модели попали в список, запросы ушли с ключами. Обе поломки — в адресах.

### 1. Z.AI: подписочный ключ на пер-токенном эндпоинте

Запрос уходил на `https://api.z.ai/api/paas/v4/chat/completions` и получал `HTTP 429, code 1113: Insufficient balance or no resource package. Please recharge.`

Ключ был из раздела **«Model & Personal Coding Plan»** — то есть подписка GLM Coding Plan, которая живёт на другом адресе: `https://api.z.ai/api/coding/paas/v4`. После смены `baseURL` (и `models.fetch`) модель ответила с первой попытки.

**Ловушка в диагностике:** симптом выглядит как проблема с деньгами, а не с конфигом. «Пополните счёт» — правда про пер-токенный счёт, которым владелец не пользуется; подписка при этом оплачена. Поэтому в пресете `zai.jsonc` дефолтом теперь стоит coding-эндпоинт, а пер-токенный — комментарием рядом, и оба адреса подписаны тарифами.

### 2. MiniMax по Anthropic-протоколу: 404 из-за отсутствующего `/v1`

Запрос уходил на `https://api.minimax.io/anthropic/messages` → **404 Not Found**. Причина: `@ai-sdk/anthropic` (3.0.78) строит путь как `` `${baseURL}/messages` `` — ровно имя метода, без версии. Значит версия обязана быть в `baseURL`: `https://api.minimax.io/anthropic/v1`.

**Наш собственный комментарий в пресете утверждал обратное** («SDK допишет к baseURL путь /v1/messages») и стоил разбирательства на живом ключе. Исправлены и `baseURL`, и комментарий; правило вынесено в спеку формата (`manuals/providersSpec.md`, врезка про `baseURL`).

**Урок:** комментарий, описывающий поведение чужой библиотеки, устаревает молча — он не участвует ни в компиляции, ни в тестах. Такие утверждения проверяются грепом по `node_modules` за десять секунд (`${this.config.baseURL}/messages` находится сразу) и должны проверяться при каждом касании соседнего кода.

### Открытый дефект, найденный этим же прогоном

На `1113` мы сделали **шесть повторов** с нарастающей паузой (7 запросов за 60 с) и показали текст «Rate limit exceeded: Insufficient balance…». Код `1113` — это не превышение частоты: повтор не пройдёт никогда, пока не пополнен счёт или не исправлен адрес. Нужна отдельная классификация «нет средств / не тот тариф» без ретраев и с подсказкой про эндпоинт. Заведено в roadmap.

## [дефект] 2026-09-24 — сервер без ключа (`"auth": "none"`) не показывал ни одной модели

Найдено живым смоуком: пример «Локальный vLLM» из спеки и засеянный `muse-glimmer-local.jsonc` — пустой список моделей,
пока не подложить фиктивный ключ через `apiKeyEnv` и `.vibe/.env`.

**Причин было четыре, и каждая сама по себе давала тот же симптом:**

- `normalizeAuth` знал только bearer/header/query и молча превращал `"none"` в bearer
- Гейт моделей требовал ключ: без ключа статус `none`, а проба каталога (`_enrichWithCatalog`) даже не запускалась
- JSON-схема не знала `"none"` — засеянный файл в редакторе подсвечен ошибкой, и это выглядело как опечатка в файле
- Поле `auth` до главного процесса не доходило вовсе: без ключа уходил `Authorization: Bearer noop`

**Грабля AI SDK: «не передать ключ» нельзя.** `loadApiKey` в `@ai-sdk/provider-utils` берёт строку как есть, даже пустую,
а при `undefined` читает `OPENAI_API_KEY`, `ANTHROPIC_API_KEY` или `GOOGLE_GENERATIVE_AI_API_KEY` из окружения. Для сервера
без ключа это значит одно из двух:

- пустой заголовок `Bearer `
- настоящий ключ пользователя, отправленный на чужой локальный сервер

Только `@ai-sdk/openai-compatible` опускает заголовок при пустом ключе, остальные три SDK — нет. Поэтому в SDK уходит метка
`KEYLESS_KEY_MARKER`, а `makeCustomFetch` снимает перед отправкой каждый заголовок, в значении которого она есть. Снимать
по значению, а не по имени: SDK называют заголовок ключа по-разному (`Authorization`, `x-api-key`, `x-goog-api-key`), а
заголовки из `headers` файла метки не несут и уходят как есть. Гейт — тест провода в `test/node/aiSdkAdapter.test.ts`: четыре
протокола, в окружении ключи-приманки. На старом коде он падал на всех четырёх.

**Гейт моделей вынесен в `dynamicKeyGate`** (чистая функция, тест в `test/browser/dynamicProviderKeyGate.test.ts`). Сервер без
ключа стоит там, где стоял бы ключ: его пробуют, и ответ решает так же — ответил (модели), не отвечает (нет моделей),
401/403 (сервер всё-таки требует ключ). Ключ из окружения ОС при `none` ничего не меняет.

**Повторная проба.** Встроенные локальные провайдеры опрашиваются каждые 5 с, а файловый пробовался только при перечитывании
набора: локальный сервер, поднятый после IDE, оставался без моделей. Теперь провайдер со статусом `error` переспрашивается
каждые 10 с (`UNREACHABLE_REPROBE_MS`); 401/403 не повторяется — его меняет только правка ключа или файла, а она и так
перечитывает набор.

**VibeIDEA** (`origin/next` @ `1d96c8f7d8`) при `none` ключ тоже не шлёт, но пускает провайдера без ключа по адресу хоста
(localhost, 127.0.0.1, ::1), а не по `none`. У нас допуск — по объявлению: сервер без ключа бывает и в локальной сети.
Расхождения (допуск, `geminiChat` шлёт ключ при `none`, явный `"bearer"` не перекрывает базовый `none` при слиянии) переданы
ей хендоффом `keyless-auth-0924-from-vibeide`.

**Живой смоук** (стенд + заглушка без ключа, 24.09): карточка «Сервер не отвечает» → заглушку подняли → через 4 с «Ключ не нужен —
сервер ответил»; модель выбрана в чате, ответ дошёл; из пяти запросов ни один не нёс ключ, ключ из `.vibe/.env` соседнего
провайдера с `apiKeyEnv` не ушёл, `headers` файла дошли до чата.

**Сделано 25.09** (запись ниже): `auth` `header`/`query`, `query`, `timeoutMs` и `headers` в пробе доходят до провода, localhost
пускается без `none`.
...
## [решение] 2026-09-25 — куда уходит ключ, локальный адрес и каталог: одно правило с VibeIDEA

**Договорённость (вариант 1).** Написанное в `auth` уходит как написано: `bearer` — `Authorization: Bearer` на любом протоколе.
Если `auth` не написан ни в одном слое, ключ уходит родным заголовком протокола ЗАПРОСА: anthropic — `x-api-key`,
gemini — `x-goog-api-key`, OpenAI — Bearer. Решает протокол запроса, а не провайдера: модель со своим `protocol` идёт своим проводом.
Одна чистая функция `keyPlacement` (`common/vibeProvidersFile.ts`) — для провода и для каталога; у VibeIDEA — `ProviderAuth.placement`.
Правило записано в наборе (`providers/README.md`) и проверяется общими векторами `testVectors/providerAuth.json` — оба продукта
гоняют один файл (`test/node/providerAuthVectors.test.ts`).

**Почему не «bearer по умолчанию».** Шлюз OpenCode читает ключ только из родного заголовка каждого протокола: на
`/chat/completions` с `x-api-key` — «Missing API key», на `/v1/messages` с Bearer — тоже (проверено запросами без ключа 25.09;
«Missing» против «Invalid» различает, увидел ли шлюз ключ). Сид `opencode-go` явно писал `"bearer"`, и восемь его моделей на
протоколе anthropic у VibeIDEA не работали. Лечит сид, а не код: `auth` у OpenCode убран (VibeBrains `ac9f729`).

**Провод.** SDK провайдера из файла получает метку `FILE_KEY_MARKER`, а не ключ: каждый SDK кладёт ключ в свой заголовок, а без ключа
читает `OPENAI_API_KEY` и родню из окружения. `makeCustomFetch` снимает заголовки с меткой и ставит ключ по `keyPlacement` —
протокол запроса известен только там (`wireOfSdkNpm`). Туда же доливаются `query` файла. Ключа нет — не уходит ничего:
`Authorization: Bearer noop` у провайдера из файла больше нет. `timeoutMs` файла — предел молчания до начала ответа, сильнее
`vibeide.llm.timeoutMs.*`. Файл, патчащий встроенного, ходит транспортом встроенного — `auth` патча там ничего не меняет.

**Локальный адрес.** `isLocalAddress` (`common/isLocalProvider.ts`) — `localhost`, `127.0.0.1`, `::1`, `0.0.0.0`. Проверка `::1` не
срабатывала ни у нас, ни у VibeIDEA: и `URL.hostname` в JavaScript, и `URI.getHost()` в Java возвращают IPv6-хост в скобках
(`[::1]`) — проверено в Node и JDK 25. Java ещё и не приводит регистр (`LOCALHOST`). Правило было записано в пяти местах
(`isLocalProvider`, три копии в `sendLLMMessage.impl.ts`, подсказка к ошибке соединения) — теперь одно.

**Localhost без `none`.** Серверу на этой машине ключ необязателен, как у VibeIDEA: задан — уходит, не задан — каталог спрашивается
без него (`dynamicKeyGate`, `localAddress`). Довод «localhost бывает прокси к облаку» снимает сама проба: прокси ответит 401, и
карточка скажет «Локальный сервер требует ключ — задайте его»; поле ключа у такого провайдера остаётся, в отличие от `none`.

**Каталог.** Адрес — `<baseURL>/models`, путь дописывается как есть, как у чата. Прежняя догадка `<baseURL>/v1/models` для базы без
`/v1` давала 404 у Gemini (`/v1beta/v1/models`; проверено запросом). Запрос каталога строит `catalogRequestOf` — те же `headers`,
`query` и ключ, у anthropic ещё `anthropic-version`; понимаются ответы `data[]` и гугловский `models[]`. Одна функция на три места:
гейт ключа, «Проверку провайдеров» и обновление каталога. 404 — «каталога нет»: модели из `static`, ключ «не проверен»; раньше 404
считался «сервер не отвечает», моделей не было вовсе, и проба повторялась каждые 10 с.

**Слияние слоёв.** `headers` и `query` сливаются по именам, как у VibeIDEA: раньше патч с одним заголовком стирал остальные.

**Живой смоук** (стенд, две заглушки на 127.0.0.1 и localhost, 25.09): карточки «Ключ не задан — локальный сервер ответил без
него» и «Локальный сервер требует ключ — задайте его»; проба и чат ушли с `?tag=smoke` и `x-team` из файла и без
`Authorization`; ответ заглушки пришёл в тред. Засеянный `zai` взял ключ владельца из глобального `~/.vibe/.env` и получил
«Ключ действителен» по `<baseURL>/models` — новый адрес каталога подтверждён и на настоящем Z.AI. Грабля стенда:
`location.reload()` из CDP окно не перезагружает — правку `providers.json` под `/Volumes` читать перезапуском копии.

**Не решено и записано в roadmap:** локальный адрес — не то же, что локальная модель. Для оптимизаций (таймауты, роутинг, промпты)
localhost-прокси к облаку «локальным» быть не должен, а сейчас окно считает его локальным, а главный процесс провайдера из файла
локальным не видит вовсе (транспорт несёт `baseURL`, а проверка читает `endpoint`).

## [решение] 2026-09-25 — локальный адрес и локальная модель разведены: `runsLocally`

Два вопроса, которые код путал: «сервер на этой машине» (ключ необязателен — `isLocalAddress`) и «модель на этой машине»
(таймауты, урезанный промпт, роутер, индикатор приватности — `isLocalProvider`). LiteLLM на localhost перед облачным Opus —
первое без второго. Поле провайдера `runsLocally` отвечает на второй вопрос, без поля — по адресу (`runsLocallyOf`).

- Решение принимается в окне, где есть запись: признак едет и на сиде настроек, и в транспорте — в главном процессе сида нет
- Патч встроенного вливается в его настройки производным полем; `_storeState` его вырезает, иначе значение засохло бы в
  блобе после удаления из файла (та же грабля split-brain, что у сидов)
- Отдельные копии «локальный» в конвертере и в повторе запроса сведены к `isLocalProvider`

## [дефект→решение] 2026-09-25 — цены и срок кэша из патча встроенного не доходили; вторая цена записи в кэш

**Дыра.** Файл с `id` встроенного провайдера (`anthropic.jsonc`, `openai.jsonc`) патчил у встроенного только выключение
моделей и подсказки провода. Цены моделей из `static` и `cacheTtl` до встроенного не доходили: сид Anthropic с
`"cacheTtl": "1h"` не включал часовой кэш, а отчёт о расходе считал по встроенной таблице.

**Правило.** Патч встроенного меняет у его моделей данные прайса — цену и срок кэша (`BuiltinModelPatch`). Поведение
(инструменты, контекст, рассуждение) остаётся встроенного, как ключ и адрес. Реестр `setBuiltinModelPatches` ставится
в окне сервисом провайдеров и в главном процессе из подсказок провода (`BuiltinWireHints.modelPatches`).

**Часовая запись.** `cache_write_1h` — отдельная цена; `getModelCapabilities` у модели с часом сам кладёт её в
`cache_write` (без объявления — 2× входа), поэтому ни один потребитель денег не знает о сроках кэша.
