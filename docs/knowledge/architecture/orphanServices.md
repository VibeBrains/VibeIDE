...
- **Почему две системы, а не одна:** persona не должна снимать constraints. Mode стоит выше персоны в стеке. См. [vibeDotfolder/settingsStack.md](../vibeDotfolder/settingsStack.md).
...
- **Правило:** `risk_score > 0.8` форсирует 🔴 независимо от judge (см. [chatUx/modesAndPolicies.md](../chatUx/modesAndPolicies.md) → confidence vs LLM-judge).
...
## ~~`telemetryService.ts`~~ — УДАЛЁН 2026-07-15; обоснование «не удалять» не подтвердилось

> **Устарело.** Позиция гласила: «**Почему не удалён:** local audit-канал нужен для GDPR self-export (Compliance секция N.2) и для оффлайн debugging. **Применение:** добавлять локальные метрики сюда». Проверка фактами 2026-07-15 её опровергла — вся `common/telemetry/` (1171 строка) удалена.

**Чем не подтвердилась:**

- **Сервиса не существовало в рантайме.** `registerSingleton` в нём не выполнялся никогда — модуль не был достижим ни от одной точки входа (доказано `npm run service-registration-check`). Незарегистрированный сервис не может обслуживать ни GDPR-экспорт, ни отладку.
- **GDPR-экспорт делает не он.** `exportAll()` / `deleteAll()` живут в `common/auditLogService.ts` — живом, с 24 импортирующими и отдельной палитрой `browser/vibeGdprPaletteContribution.ts`. Отметка «GDPR export ✅ / delete ✅» в roadmap относится к аудит-логу.
- **Экспорта через `vibe doctor` не было.** В `scripts/vibe-doctor.js` телеметрия не упоминается вообще.
- **Метрик в него никто не писал.** `IVibeideTelemetryService` не инжектил ни один живой сервис; единственные потребители — такие же мёртвые `adaptiveRouter` и `speculativeEscalationValidator`.

**Урок важнее самого удаления:** запись «почему не удалён» читается как принятое решение и **блокирует чистку на годы**. Здесь она фиксировала *намерение*, а не факт, и опиралась на комплаенс — то есть звучала максимально весомо. Обоснование в доке — такая же гипотеза, как всё прочее: проверять по фактам (кто импортирует, кто регистрирует, кто пишет события). Отсюда правило «раскопки перед удалением» в `CLAUDE.md`: проверять и записанное решение тоже.

**Если тема вернётся:** замысел (таксономия событий, storage с gzip и ротацией 30 дней, аналитика) сохранён в [inheritedPrototypes.md](inheritedPrototypes.md) → «Вектор 1». Правило оттуда: сперва писать события из живого пути, потом строить поверх них поправки — обратный порядок эту подсистему и убил.

**Что осталось в силе:** cloud telemetry в VibeIDE отсутствует по дизайну (Фаза 1 — «телеметрия отключена / локальная»); сетевой sink не подключать без явной отдельной фичи и user opt-in.
