...
**Контекст:** обзор домена `tool-system/`. Слой встроенных тулов — это поверхность, которую LLM видит как `read_file`, `run_command`, `glob`, `grep` и т.д.
...
**Связано:** остальные записи домена перечислены в [индексе базы знаний](../README.md) (раздел `tool-system/`) — отдельного списка здесь намеренно нет, чтобы не плодить второй индекс.
