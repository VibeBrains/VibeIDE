...
| [checkpointWorkspaceSnapshot.md](architecture/checkpointWorkspaceSnapshot.md) | Снимок рабочей папки в чекпоинтах: дерево git через **временный индекс** (настоящий не трогается), засев индекса копией `.git/index` — иначе `git add -A` перехэширует всё дерево; `git stash create` не годится (не берёт untracked, а это и есть вывод терминала); снимок на пользовательском чекпоинте, не на каждой правке; восстановление **только за подтверждением** — оно перезаписывает нетронутые чатом файлы и удаляет созданные после чекпоинта; чистый план восстановления отдельно от запуска git |
...
| [modelPricing.md](architecture/modelPricing.md) | Прайс моделей: `getModelCapabilities().cost` (USD/1M), `{0,0}`=«неизвестно»≠$0; была ловушка «per-token → врёт в 1e6 раз», закрыта 06.08 через `catalogPricing.ts`; ступень `long_context` — механизм был, данных не было: профили заполнены, `pricing.overrides` каталога переводится в множители, временны́е скидки не берутся; токены оркестратора (Sakana Fugu) приходят сверх видимых в конце ответа и мимо SDK — две формы, никогда не складываются |
| [roleMoneyCeilings.md](architecture/roleMoneyCeilings.md) | Потолки роли в долларах рядом с токенными: `perRun` превращается в квоту токенов при запуске (курс известен только там), `perDay` проверяется по журналу; неизвестная цена не ноль — такие прогоны не в сумме и названы в отказе. `.vibe/agents.json` тут ни при чём: это реестр внешних ACP-агентов, другой кошелёк |
| [floatingModelIds.md](architecture/floatingModelIds.md) | Плавающий id ведёт к разным моделям в разные дни и не падает ошибкой при перенацеливании: явный алиас (`alias_target`, 16 записей) и датированный снапшот (`canonical_slug` ≠ id, 294 записи). Пометка в списке моделей, чтобы квирк вешали на снапшот |
| [taskBill.md](architecture/taskBill.md) | Счёт за задачу, а не за запрос: прогоны одной цели складываются в цепочку (`replayOfRunId`/`escalatedFromRunId`), брошенные попытки считаются отдельной строкой, медиана и девятый дециль вместо среднего; «доведено» = завершившийся прогон, вердикта человека журнал не хранит |
...
| [layerSplitElectronBrowser.md](architecture/layerSplitElectronBrowser.md) | **[рецепт]** Сплит desktop-only сервиса: контракт в `common/`, класс+`registerSingleton`+`IMainProcessService` в `electron-browser/`, перевязка в `workbench.desktop.main.ts` (апстримный файл — цена осознанная). **Ловушки:** `browser/` тоже запретная зона; список нарушителей грепом врёт (спрашивать чекер); регистрация «попутной» загрузкой умирает молча. **Базлайн 3 ОПРОВЕРГНУТ (15.08): стало 0** — «неустранимый апстрим» `nativeBrowserElementsService.ts` оказался зомби эпохи 1.106 (его нет ни в 1.118.1, ни в 1.133.0); удалён, исключение снято, чек зелёный. Урок: «пришёл с Initial import» ≠ «апстримный» — проверять `git cat-file -e <тег>:<путь>`. Смоук: реестр живого рантайма через CDP + доказательство сквозь IPC |
...
| [projectHooks.md](architecture/projectHooks.md) | Хуки проекта: команды вокруг цикла агента (`preToolUse`/`postToolUse`/`turnEnd`), отказ — код выхода `2` (единица не блокирует: её возвращает любой сломанный скрипт), три замка перед запуском (настройка + доверие к папке + разбор файла), битая запись не применяется частично Чужой прецедент (portal-ai-plugins, issue #10): JSON-ответ хука не по схеме хоста молча не блокирует — у нас решает код выхода, а отказ может маршрутизировать, называя, что делать вместо |
| [telegramBridge.md](architecture/telegramBridge.md) | Мост в Telegram: **свой бот у каждого, без нашего сервера** (polling посредника не требует, релей пропускал бы переписку через нас); **поллер в main, а не в окне** — два `getUpdates` по одному боту дают `409 Conflict`, а наш window-lock живёт per-workspace и тут не помогает; **HTML-режим вместо MarkdownV2** (одна неэкранированная скобка в коде теряет ответ целиком); свой прокси через выделенную `createProxyDispatcher` + грабля «диспетчер на каждый запрос = утечка пула»; привязка чата обязательна, молчание = отказ |
...
| [commandPaletteTitles.md](ui/commandPaletteTitles.md) | «VibeIDE: VibeIDE: …» в палитре: категория и заголовок — разные поля; 40 дублей, 21 команда без группы, 17 английских заголовков; гейт `command-titles-check` |
...
| [chatInterruptAndInject.md](chatUx/chatInterruptAndInject.md) | Дубль `tool_call id` после abort mid-tool-call (HTTP 400) — дедуп в `prepareMessages_openai_tools`; дизайн «подмешать контекст к следующему хопу» без прерывания; **правило: живой UI-статус в треде — транзиентом, не персистентным сообщением** (инвариант `messages[length-1]`, буфер notice до idle) | ; **пробуждение агента фоновой командой** (занятой тред — очередь, простаивающий — новый ход; наоборот нельзя ни в одну сторону) ; **чек-лист «проверь глазами»** (состояние треда, не messages; непроверенное ≠ работающее)
...
| [voiceDelegationLayer.md](voice/voiceDelegationLayer.md) | Ориентир, не план: голосовой слой держит разговор и передаёт задачу агенту (GPT-Live-1: full-duplex, рассуждение и инструменты — у бэкенда, $0,05/мин). Что подтверждено, что слух, что по данным VibeIDEA; почему не интегрируем (облако, русский не заявлен, открытого full-duplex с русским нет); что взять в дизайн — граница «голос без инструментов», задача явным текстом, «замолчать» ≠ «отменить ход агента» |
...
| [dmgInstallerWindow.md](build/dmgInstallerWindow.md) | Окно установки .dmg: релиз собирал голый образ `hdiutil -srcfolder` (стоковые фоны апстрима не отгружались вовсе). Теперь `scripts/build-dmg-macos.sh` — раскладка через Finder, фон из знака, гейт по sha256 фона и наличию `.DS_Store`. Разный ответ замера на одинаковых плашках — ошибка окна замера |
...
| [githubActionsPins.md](build/githubActionsPins.md) | GitHub Actions: `checkout` и `setup-node` — v6 по SHA из `pr.yml` во всех workflow (82 ссылки, 33 файла). Что сверено перед сменой мажора: раннер не старше v2.327.1, учётные данные checkout v6 в `$RUNNER_TEMP` (`git push` в `run:` не задет), автокэш setup-node только при `packageManager`. Грабля — ссылка в кавычках: комментарий становится частью строки. Вторая волна: остальные 18 ссылок — тоже по SHA, пять actions, работавших на Node 20, подняты до мажоров на Node 24 (сверено: SHA против тегов, `runs.using`, входы шагов, `github-script` v9 без `require('@actions/github')`, dotfiles в артефакте Pages). Проверка незакреплённых `uses:` — только с якорем в начале строки: без него ловятся `statuses:` и `causes:` |
...
| [precommitHygiene.md](gitAndTools/precommitHygiene.md) | `tsx`-раннер hygiene/eslint, фильтры Unicode/indentation для vibeide, lint-staged без eslint, `--no-verify` на больших коммитах. **+ [foot-gun]** фильтры **каскадные** (`all ⊃ eol ⊇ indentation ⊃ copyright ⊃ typescript`) — исключение в раннем фильтре снимает и все последующие проверки (`!` в copyright отключил ESLint); shebang vs заголовок на строке 0 — ложная дилемма (shebang рудимент); линт видит файл, только когда он staged **+ [foot-gun] (2026-09-11)** хук и подмодули: lint-staged и hygiene подмодули не видят; git экспортирует в хук относительный `GIT_INDEX_FILE`, и дочерний git в подмодуле работает не с тем репозиторием — очищать `git rev-parse --local-env-vars`; хук доказывается только настоящим `git commit` |
...
| Файл | О чём |
|---|---|
| [postUpstreamFailures.md](testing/postUpstreamFailures.md) | **[реестр+долг]** Семь падающих unit-тестов после обновления базы на 1.133 (замер 31.08.2026: 26841 passing / 7 failing). Четыре — апстримный мастер Copilot, ждущий хост `vscode.dev` и протокол `vscode` при наших `vibeide`; один — хрупкая дата в дашборде; два — тайминг MCP и размеры в styleOverrides. Ни одно не связано с правкой, которая их обнаружила. Плюс грабля: `out/` старше исходников прогоняет старый код и красит зелёным |
...
| [quickFixRepairLoop.md](toolSystem/quickFixRepairLoop.md) | Быстрые исправления редактора применяются после правки агента вместо починки за токены; LLM-фолбэка в туле намеренно нет (в цикле это работа самой модели); применённое сообщается модели, иначе она «чинит» уже целое. **Грабля:** вставка нулевой длины формально «заканчивается до» содержащего её диапазона — «add import» + «organise imports» продублировали бы импорт; пустой диапазон конфликтует включительно по границам. Попутно закрыта течь `modelRef` в `detectErrorsInFile` |
| [preApplyVerification.md](toolSystem/preApplyVerification.md) | Сверка файла перед применением правки агента: подпись `{hash, source}` при чтении, проверка перед записью. Хэшируется ВЕСЬ файл, а не отданная страница; `source` (буфер/диск) обязателен — сравнение буфера с диском отказывало бы на каждом несохранённом файле; подпись обновляется после каждой записи, иначе цепочка `edit → edit` блокирует сама себя; при расхождении базлайн сбрасывается на «сначала прочитай». Гоча: предикат-хелпер не сужает union, порядок гейтов — до `initializeModel` |
...
| [externalServicesParity.md](agentCollaboration/externalServicesParity.md) | Разбор присланных сервисов → паритет с VibeIDE. Алгоритм разбора; Multica/Paperclip = паритет с Vibe Agents (маппинг «их фича → наш сервис»); единственное реально новое — денежный per-agent бюджет (Paperclip); PCLink = AGPL → только паттерн. **Skales (09.08) = репозиторий-витрина**: BSL 1.1 запрещает конкурирующий продукт, а выложенное дерево — снимок v7 при продукте 12.7.1, то есть смотреть нечего и по праву, и по факту; распознаётся за один запрос — версия в `package.json` против версии релиза. Пользовательская проекция — `docs/references-v1/vibeide-vs-alternatives.md` **Метод разбора дайджеста (09.08):** скептик опроверг 6 тем из 12 — мимо шли не факты, а выводы «нам это нужно»; требовать первоисточник, проверять трудоёмкость, заканчивать грепом по своему репо. Побочно найдено 7 дефектов в нашем коде.  **repowise (14.08, AGPL-3.0)**: код не берём, взята механика — обратимость сжатия вывода (`expand_output`), профили линтера/сборки/поиска, связанность правок и история починок из git. Греп опроверг мой же первый вывод: «distill» у нас был на 70% сделан под другим именем (`commandOutputCompressor`), заодно вскрылся дефект — `npm run <script>` целиком считался тестом. Гэп-механики: массовый коммит связанности не создаёт; починка определяется формой коммита, а не словом `fix`; «часто меняется» ≠ «часто ломается»; абсолютная шкала риска непортируема (для нашего `editRiskScoringService` — гипотеза, не дефект). Грабли: настоящий NUL-байт в TS-строке и `\w`/`\b` мимо кириллицы |
| [metricOptimizationLoop.md](agentCollaboration/metricOptimizationLoop.md) | Цикл «правка → замер → оставить/откатить» по разбору karpathy/autoresearch: вердикт выносит инструмент, а не модель; фиксированный бюджет на попытку и закрытый от правок измеритель; «не измерили» ≠ «стало хуже»; метрика по контракту, а не догадкой |
| [learningWorkspace.md](agentCollaboration/learningWorkspace.md) | Обучение, переживающее конец треда: прогресс файлами в `.vibe/learning/`, миссия как гейт (нет ответов на четыре вопроса — нет урока), сложность выбирает код по следам уроков, а не модель по впечатлению от собственного урока; повтор застревания перекрывает чистую серию; пустой `stuck` — утверждение, а не пропуск. Что из разбора `/teach` не взято и почему |
| [acpHost.md](agentCollaboration/acpHost.md) | ACP-хост и живой прогон с Claude Code: клиентскую файловую систему он **не вызывает вовсе** (заявка спецификации не подтвердилась) — ворота стоят на `session/request_permission` и кадрах `tool_call`, где приезжает готовый дифф ДО записи; грабля «дифф не в завершающем кадре» (журнал, ждущий его там, не запишет ни одной правки); метки `CLAUDECODE` мешают старту; `authenticate` адаптер не реализует |
| [agentSkillsSpec.md](agentCollaboration/agentSkillsSpec.md) | Спецификация Agent Skills: закрытый список полей верхнего уровня (эталонный валидатор **отвергает** лишние), наши расширения — под `metadata` (парсеры уже читают их через `^\s*поле:`), два найденных дефекта совместимости: обязательный `vibeVersion` и блочное описание, дававшее в каталоге палочку |
...
| [pipelineContextHandoff.md](agentCollaboration/pipelineContextHandoff.md) | Пайплайн: шаг получает ход работы — прошлые шаги строкой до 300 символов со статусом и вердиктом ревью, предыдущий целиком; «дневник» — вопрос длины, а не памяти, а шаг, знающий только последнее слово, решает заново (Cognition, 2025). Ревьюер получает задачу, критерий и файлы без пересказа исполнителя (`composeReviewGoal`); пересказ — настройкой `vibeide.pipeline.reviewerSeesStepSummary` для сравнения, режим пишется рядом с вердиктом; **замер не проводился** |

### [security/](security/) — безопасность конфигов и рантайма

| Файл | О чём |
|---|---|
| [owaspAgenticTop10.md](security/owaspAgenticTop10.md) | Сверка ASI01–ASI10 с нашими guard'ами: 8 из 10 закрыто, ASI07 неприменим, единственный гэп — отравление контекста через RAG-индекс и историю чата. Разбор Microsoft agent-governance-toolkit (не применим: он про изоляцию серверных агентов) · **`privacy.strict` обещал блокировку, которой нет** (гейт FIM читал несуществующее `privacyMode` через `as`-каст → вечный `undefined`; описание настройки — это обещание) |
| [auditTrailIntegrity.md](security/auditTrailIntegrity.md) | Журнал вне зоны файловых инструментов агента, цепочка хешей против вырезанной строки, обязательный актор события и метка записи в общую `.vibe/` — по разбору инцидента OpenAI/Hugging Face |
| [externalAccessRevoke.md](security/externalAccessRevoke.md) | Отзыв папки посреди прогона: раньше следующий же вызов инструмента показывал тот же диалог разрешения, и агент отменял отзыв окном. Теперь отзыв помнится до конца сессии, накрывает вложенные пути, снимается только явной выдачей человеком, а прогон читает причину словами. Почему пятый предохранитель тут лишний. Сроки разрешения: вид «на задачу» до конца хода, срок сессионного в минутах, проверка при чтении |
| [mcpServerWorkingDir.md](security/mcpServerWorkingDir.md) | MCP-сервер запускался без `cwd` и наследовал каталог IDE: процесс один на все окна, и сервер памяти VibeMemory, определяющий проект по месту запуска, в dev-запуске писал бы всё в проект VibeIDE. Поле `cwd`, находка сервера памяти с домашним каталогом — запись без `project` отклоняется. Грабли: `headers` из записи не отправляются, `env` перекрывается окружением IDE |
| [verificationEdits.md](security/verificationEdits.md) | Ход правил то, что его проверяет (тесты, фикстуры, обвязка): правило `no-verification-edit` — сигнал, а не запрет, и выключено по умолчанию, иначе мешает каждой задаче про тесты. Первая формулировка «закрыть тесты от записи» не сработала бы: у VERIFY-GATE нет списка своих файлов, а `deny_write` уже есть |
| [mcpClientRegistration.md](security/mcpClientRegistration.md) | Своего OAuth-клиента к MCP у нас нет — вход идёт механизмом редактора, поэтому CIMD/DCR сегодня забота апстрима. Решение на будущее: собственный gateway рождается на **CIMD**, DCR не реализуем (спека 2026-07-28 называет его устаревшим, даты удаления нет). Что закладывать сразу: `resource` по RFC 8707 в оба запроса, PRM как единственный discovery, обе схемы метаданных, валидация `iss` (SHOULD → MUST в будущей ревизии). DPoP и WIF — без дат |
| [exfiltrationSequence.md](security/exfiltrationSequence.md) | Пара «чтение секрета → сетевой вызов» как сигнал: четыре предохранителя судят по одному событию и последовательности не видят. Работает на уже собираемом следе вызовов; НЕ останавливает ход — порогов первоисточник (GTIG) не публикует, и стоп по придуманному порогу дороже пропуска. Дедупликация по окну, а не навсегда: та же пара через час тревожнее первой |
| [configGuard.md](security/configGuard.md) | Config Guard — статический скан `.vibe/providers.json` и `mcp.json` при загрузке (13 правил из AgentShield под поверхность VibeIDE): non-https/raw-IP endpoint, хардкод секретов, `curl\|sh`/`npx -y`/`--no-sandbox` в MCP. Чистый `vibeConfigGuard.ts`, `vibeide.configGuard.*` (warn/block), команда `vibeide.configGuard.showFindings`. Что НЕ дублирует: secretDetection + promptGuard |
| [agentPathPolicy.md](security/agentPathPolicy.md) | Путь агента: проверка и действие называют один файл. Лексический слой (`..`) — в валидации, физический (симлинки) — в каждом файловом инструменте; висячая ссылка — отказ; корни разворачиваются тоже (корень-ссылка на машине владельца); запреты — по всем именам файла, разрешения — по месту; запреты не зависят от регистра и сравниваются в NFC; поиск агента не ходит по ссылкам. Аудит: `deny_read`/`allow_read` не применялись нигде, удаление и создание обходили `constraints.json` |
| [shellLineVerdict.md](security/shellLineVerdict.md) | Строка терминала: опасна связка, а не половина — `curl … \| sh` судится как композиция «загрузчик → интерпретатор, берущий программу из pipe» (`\| python3 -m json.tool` — не тревога, иначе приучит жать не глядя); код причины общий с VibeIDEA, их тест-вектор у нас дословно. Мёртвый уровень `high` в `_detectCommandDanger`: `git push -f`, `fdisk`, `curl \| sh` не давали никакого сигнала. Обёртки с опциями-значениями, `#` не комментарий (cmd.exe), дисковая утилита на просмотр — без диалога |
| [skillApproval.md](security/skillApproval.md) | Одобрение скилла по отпечатку всего каталога (текст, скрипты, вложения): модель видит вывод скриптов, но не код. Гейт — там, где текст скилла идёт в модель (`convertToLLMMessageService`: список, подсказки, раскрытие `/skill:`); неодобренные — одним числом без имён; отказ помнится по версии, иначе цикл агента переспрашивал бы каждый шаг. Одобрения — в профиле (`MACHINE`), настройка — `APPLICATION`: репозиторий не может ни принести одобрения, ни выключить требование. Происхождение — по всему каталогу; исполняемые файлы — в окне одобрения, а не всплывающим окном |
