...
| [layerSplitElectronBrowser.md](architecture/layerSplitElectronBrowser.md) | **[рецепт]** Сплит desktop-only сервиса: контракт в `common/`, класс+`registerSingleton`+`IMainProcessService` в `electron-browser/`, перевязка в `workbench.desktop.main.ts` (апстримный файл — цена осознанная). **Ловушки:** `browser/` тоже запретная зона; список нарушителей грепом врёт (спрашивать чекер); регистрация «попутной» загрузкой умирает молча. **Базлайн 9 — зелёным быть не может** (3 апстрим + 6 отложенных). Смоук: реестр живого рантайма через CDP |
