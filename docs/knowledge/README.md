...
| [projectHooks.md](architecture/projectHooks.md) | Хуки проекта: команды вокруг цикла агента (`preToolUse`/`postToolUse`/`turnEnd`), отказ — код выхода `2` (единица не блокирует: её возвращает любой сломанный скрипт), три замка перед запуском (настройка + доверие к папке + разбор файла), битая запись не применяется частично |
