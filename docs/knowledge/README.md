...
| [modelPricing.md](architecture/modelPricing.md) | Прайс моделей: `getModelCapabilities().cost` (USD/1M), `{0,0}`=«неизвестно»≠$0; была ловушка «per-token → врёт в 1e6 раз», закрыта 06.08 через `catalogPricing.ts`; ступень `long_context` — механизм был, данных не было: профили заполнены, `pricing.overrides` каталога переводится в множители, временны́е скидки не берутся |
