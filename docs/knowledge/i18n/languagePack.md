...
## [грабля] Русский языковой пакет перекрывает `package.nls.json` — переименование темы не доходит до интерфейса

**Контекст:** 2026-08-10, попытка переименовать темы `Dark 2026` / `Light 2026`.

**Симптом:** метка темы правится в `extensions/theme-defaults/package.nls.json`, поле `name` в самом
файле темы и лейбл в `product.json` — а в интерфейсе остаётся старое название. Ошибок нет, сборка
проходит, всё выглядит применённым.

**Причина:** `defaultLocale: "ru"` в `product.json`, и файл перевода
`extensions/vscode-language-pack-ru/translations/extensions/vscode.theme-defaults.i18n.json`
**сильнее** исходного `package.nls.json`. Правка исходника видна только при locale=en. Тот же слой
перекрывает и экран приветствия (`main.i18n.json`, секции `theme_picker` / `theme_picker_small`).

**Правило:** **названия тем — имена собственные, они не переводятся.** Переводить нужно описание
расширения, а не имя. Переименовал тему — правь пару: `package.nls.json` (или `product.json`)
**и** соответствующий `*.i18n.json` русского пакета, иначе правка не приземлится.

**Смежное:** идентификатор темы (`id` в `package.json`) — отдельная сущность; менять `label`/`name`
безопасно, тема у выбравших её пользователей не слетает. Менять `id` — слетает.

**Формат правки файлов переводов:** они огромны, и перезапись через `json.dumps` переформатирует
файл целиком, утопив содержательную правку в шуме диффа. Править точечной текстовой заменой с
последующей проверкой `json.loads`.
