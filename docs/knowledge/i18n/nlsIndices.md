...
- Кнопки нотификаций с **`&&`** буквально — **отдельный** фикс в `notificationsViewer.ts` (см. [languagePack.md](languagePack.md) → мнемоники).
...
### Корень системного дрейфа (найден 2026-07-11): vibe-dev перегенерировал метаданные из свежего src при старом out

**Симптом-маркер:** диалог сохранения показывает буквально «Сохранить изменения, внесенные в {0}?» — сырой плейсхолдер. Это detail-строка диалога, получившая **чужой** перевод (строку с `{0}`) из-за сдвига индексов на +1: в `src` добавили один `localize(...)` (новый коммит), `out/*.js` остался старым, а `scripts/vibe-nls-extract.ts` **на каждый запуск `./run-dev.sh`** (даже без `--compile`) перезаписывал `out/nls.{keys,messages}.json` из **текущего** src → все ключи после точки расхождения сдвинуты. Существующая защита (хэш метаданных в пути clp-кэша, `src/vs/base/node/nls.ts`) честно инвалидировала кэш и пересобрала его **из уже сдвинутых** метаданных — она ловит устаревший кэш, но не расхождение «метаданные ↔ запечённые в JS индексы».

**Фикс (main→next, `17205c50`):** `scripts/vibe-dev.sh`/`.bat` запускают extract **только если `out/nls.messages.json` отсутствует** — gulp-компиляция сама эмитит согласованные метаданные (`build/lib/nls.ts`, preserveEnglish). Лечение уже поражённого профиля: `./run-dev.sh --compile` (полный compile переиздаёт JS и метаданные из одного дерева; clp-кэш пересоберётся сам по новому хэшу).

**Вторая линия защиты (TODO, roadmap VA.6):** в `vibe-nls-extract.ts` сверять вычисленные start-индексы модулей с запечёнными в `out/**/*.js` (`min` индекс по regex `localize2?\((\d+)`) и отказываться писать при расхождении, требуя `npm run compile`.

**Диагностика на будущее:** взять «съехавшую» строку → найти её индекс в живом clp-кэше профиля (`.../clp/<hash>.ru/.../nls.messages.json`) → сравнить с `localize(<N>, ...)` в соответствующем `out/**/*.js`; разница «длина `out/nls.keys.json` vs количество запечённых вызовов» = величина сдвига; бинарный поиск по модулям находит точку расхождения.
...
- **`&&` в кнопках уведомлений** — патч `mnemonicButtonLabel` в `notificationsViewer.ts` (см. [languagePack.md](languagePack.md)).
