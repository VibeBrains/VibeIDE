...
- **Наборы `test/node/**`, которым нужен настоящий Node** (`llmMessageAbort`, `aiSdkAdapter`, та же паритетная проверка
  графа) — общий помощник `test/node/nodeOnly.ts` (`skipInElectronRenderer`) первым в `suiteSetup`, модули пути
  отправки — динамическим импортом ПОСЛЕ него. Статический импорт такого модуля падает ещё при загрузке файла
  (рендерер не резолвит подпуть `@mistralai/mistralai/core.js`) и останавливает весь `./scripts/test.sh` — пропуск
  внутри набора уже не спасает. `suiteTeardown` при пропуске всё равно выполняется: сервер закрывать с проверкой.
