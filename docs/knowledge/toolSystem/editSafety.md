...
**Контекст:** 2026-06-11 (инцидент minimax, `xmlToolFormatIncidents.md`). Когда модель пере-эмитит большой файл целиком через `rewrite_file`, её вывод может обрезаться → тул молча перезаписывает файл огрызком («урезался до 11 строк»). У minimax это запустило каскад: rewrite режет → модель уходит в одноразовые `__patch_*.py`-скрипты.
