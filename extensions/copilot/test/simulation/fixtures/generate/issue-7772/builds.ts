...
class Builds {
	...
	private async getBuildArchiveName({ runtime, commit }: IBuild): Promise<string> {
		switch (runtime) {
			...
			// - macOS: just the name, nice! (e.g. VibeIDE-darwin.zip)
			// - Linux: includes some unix timestamp (e.g. code-insider-x64-1639979337.tar.gz)
			// - Windows: includes the version (e.g. VibeIDE-win32-x64-1.64.0-insider.zip)
			case Runtime.DesktopLocal:
				switch (platform) {
					case Platform.MacOSX64:
						return 'VibeIDE-darwin.zip';
					case Platform.MacOSArm:
						return 'VibeIDE-darwin-arm64.zip';
					case Platform.LinuxX64:
					case Platform.LinuxArm:
					...
					case Platform.WindowsArm: {
						...
						return platform === Platform.WindowsX64 ? `VibeIDE-win32-x64-${buildMeta.productVersion}.zip` : `VibeIDE-win32-arm64-${buildMeta.productVersion}.zip`;
					}
				}
		}
	}
	...
	async getBuildName({ runtime, commit }: IBuild): Promise<string> {
		switch (runtime) {
			...
			// - Windows: includes the version (e.g. VibeIDE-win32-x64-1.64.0-insider)
			case Runtime.DesktopLocal:
				switch (platform) {
					case Platform.MacOSX64:
					case Platform.MacOSArm:
					...
					case Platform.LinuxX64:
						return 'VibeIDE-linux-x64';
					case Platform.LinuxArm:
						return 'VibeIDE-linux-arm64';
					case Platform.WindowsX64:
					case Platform.WindowsArm: {
						const buildMeta = await this.fetchBuildMeta({ runtime, commit });

						return platform === Platform.WindowsX64 ? `VibeIDE-win32-x64-${buildMeta.productVersion}` : `VibeIDE-win32-arm64-${buildMeta.productVersion}`;
					}
				}
		}
	}
